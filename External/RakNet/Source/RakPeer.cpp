...
//	Packet **threadPacket;
	...
	*/
	...
	do
	{
		...
		if (packet==0)
			return 0;
		// PunkBuster Proprietary SDK Code - CONFIDENTIAL
		// Copyright 2013 - Even Balance, Inc.
#ifdef __WITH_PB__
		// PunkBuster Packets
		if ( memcmp ( packet->data, "\xff\xff\xff\xffPB_", 7 ) == 0 )
		{
			return packet ;
		}
#endif
...
bool ProcessOfflineNetworkPacket( SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSocket2* rakNetSocket, bool *isOfflineMessage, RakNet::TimeUS timeRead )
{
	...
	if ( length > 7 && memcmp ( data, "\xff\xff\xff\xffPB_", 7 ) == 0 )
	{

		RakNet::BitStream inBitStream( (unsigned char *) data, length, false );

		packet=rakPeer->AllocPacket(length, _FILE_AND_LINE_);
		memcpy(packet->data, data, length);
