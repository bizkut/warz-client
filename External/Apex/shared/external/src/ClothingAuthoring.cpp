...
ClothingAuthoring::Simulation::ClothingAsset::~ClothingAsset()
{
	...
	mCurrentActorDesc(0)
{
	...
	if (mSimulation.cudaContextManager == NULL)
	{
		...
		physx::pxtask::CudaContextManagerDesc cudaContextManagerDesc;
		if (mSimulation.cudaContextManager == NULL && physx::pxtask::getSuggestedCudaDeviceOrdinal(_mApexSDK->getPhysXSDK()->getFoundation().getErrorCallback()) != -1)
		{
			mSimulation.cudaContextManager = physx::pxtask::createCudaContextManager(_mApexSDK->getPhysXSDK()->getFoundation(), cudaContextManagerDesc, profileZoneManager);
		}
#endif
	}
#endif
	...
	mSimulation.previewScene = _mApexSDK->createAssetPreviewScene();
...
void ClothingAuthoring::connectPVD(bool toggle)
{
	...
	if (pvd != NULL)
	{
		...
		if (!pvd->isConnected())
		{
			...
			{
				flags |= physx::debugger::PvdConnectionType::Debug;
			}
			if (mConfig.simulation.pvdProfile)
			{
				flags |= physx::debugger::PvdConnectionType::Profile;
			}
			if (mConfig.simulation.pvdMemory)
			{
				flags |= physx::debugger::PvdConnectionType::Memory;
			}
...
void ClothingAuthoring::addMaterialToLibrary(const char* libName, const char* matName)
{
	...
	error = arrayHandle.getChildHandle(materialLibrary, "verticalStiffnessScaling.range", elementHandle);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	error = elementHandle.setParamF32(1.0f);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	...
	error = arrayHandle.getChildHandle(materialLibrary, "horizontalStiffnessScaling.range", elementHandle);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	error = elementHandle.setParamF32(1.0f);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	...
	error = arrayHandle.getChildHandle(materialLibrary, "bendingStiffnessScaling.range", elementHandle);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	error = elementHandle.setParamF32(1.0f);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	...
	error = arrayHandle.getChildHandle(materialLibrary, "shearingStiffnessScaling.range", elementHandle);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
	error = elementHandle.setParamF32(1.0f);
	PX_ASSERT(error == NxParameterized::ERROR_NONE);
...
#endif
		NxParameterized::setParamBool(*actorDesc, "useStiffSolver", mConfig.apex.simulationType == 0);
		...
		if (mSimulation.actorCount > (int)mSimulation.actors.size())
		{
			for (int i = (int)mSimulation.actors.size(); i < mSimulation.actorCount; i++)
			{
				...
				for (size_t j = 0; j < mSimulation.assets.size(); j++)
				{
					...
					{
						// I don't know what is the proper solution for previewScene here ! 
						physx::apex::NxApexAssetPreview* apexPreview = mSimulation.assets[j].apexAsset->createApexAssetPreview(*previewDesc, mSimulation.previewScene);
...
void ClothingAuthoring::setMatrices(const physx::PxMat44& viewMatrix, const physx::PxMat44& projectionMatrix)
{
	if (mSimulation.apexScene != NULL)
	{
		...
		mSimulation.previewScene->setCameraMatrix(viewMatrix.inverseRT());
	}
}
...
bool ClothingAuthoring::addAssetToScene(physx::apex::NxClothingAsset* clothingAsset, physx::apex::IProgressListener* progress)
{
	if (mSimulation.apexScene == NULL)
	{
		return false;
		...
		for (size_t j = 0; j < mSimulation.assets.size(); j++)
		{
			...
			if (j < mSimulation.actors[i].actors.size())
			{
				...
				if (timeout < 0.0f)
				{
					...
					{
						physx::PxMat44(physx::PxMat33(rot1), pos1),
						physx::PxMat44(physx::PxMat33(rot2), pos2)
					};
					...
					teleportMode = physx::apex::ClothingTeleportMode::TeleportAndReset;
				}
				timeout -= 1.0f / 50.0f;
#endif
...
{
	if (mConfig.painting.valueFlag != flags)
	{
		mDirty.workspace = true;
...
#define ADD_PARAM_OLD(_CONFIG, _PARAMNAME, _PARAM) _CONFIG##Old[_PARAMNAME] = &_PARAM
	...
	//ADD_PARAM_NEW(mIntConfiguration,  mConfig.apex.simulationType);
