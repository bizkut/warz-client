...
public:
	...
	bool  addAssetToScene(physx::apex::NxClothingAsset* asset, physx::apex::IProgressListener* progress);
	void  handleGravity();
	...
	{
		return mMeshes.subdivideHistory[index].submesh;
	}
	int getSubdivisionSize(int index)
	{
		return mMeshes.subdivideHistory[index].subdivision;
	}
	...
	void  setSimulationType(int on)
	{
		mDirty.workspace |= mConfig.apex.simulationType != on;
		mConfig.apex.simulationType = on;
		mState.needsRestart = true;
	}
	int   getSimulationType() const
	{
		return mConfig.apex.simulationType;
	}
	...
	{
		return mConfig.painting.valueFlag;
	}
...
private:
	...
	struct configApex
	{
		...
		void init()
		{
			...
			forceEmbedded = false;
			simulationType = 1; // useStiffSolver off by default
		}
	};
