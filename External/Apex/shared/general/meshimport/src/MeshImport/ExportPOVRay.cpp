...
const MiF32 * getFloat3(const MiF32 *texel)
{
	static MiF32 pos[3];
	pos[0] = texel[0];
	pos[1] = texel[1];
...
void exportSubMesh(Mesh *m,SubMesh *sm,FILE_INTERFACE *fph)
{
	char scratch[512];
	MESH_IMPORT_STRING::sprintf_s(scratch,512,"%s_%s", m->mName, sm->mMaterialName );
