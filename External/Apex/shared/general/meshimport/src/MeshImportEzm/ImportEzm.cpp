...
public:
		...
		case NT_MATERIAL:
			{
				...
				const char *meta_data = getAttribute("meta_data",argc,argv);
    ...
    if ( svalue )
    {
  		switch ( mType )
  		{
			default:
              ...
              char scratch1[2048];
              char scratch2[2048];
              MESH_IMPORT_STRING::strcpy_s(scratch1,2048,mCtype);
              MESH_IMPORT_STRING::strcpy_s(scratch2,2048,mSemantic);
...
#endif
              {
                mCallback->importIndexedTriangleList(mCurrentMesh.Get(),mCurrentMaterial.Get(),mVertexFlags,mVertexCount,mVertices,mIndexCount,(const MiU32 *)mIndexBuffer );
              }
  					}
  ...
  {
    ProcessNode(elementName,argc,argv);
    MiI32 acount = argc/2;
