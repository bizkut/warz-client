...
PxU64 D3DX11::getInputHash(const D3D11_INPUT_ELEMENT_DESC* inputDesc, UINT numInputDescs)
{
	...
	const PxU8 bitsPerElement =  physx::PxMin((PxU8)((sizeof(PxU64) * 8) / numInputDescs), (PxU8)1);
	...
	for (UINT i = 0; i < numInputDescs; ++i)
	{
		if(inputDesc->SemanticName)
		{
			...
			if (physx::PxStricmp(inputDesc[i].SemanticName,      "TEXCOORD") == 0)
			{
				flag = USE_TEXCOORD;
			}
			else if (physx::PxStricmp(inputDesc[i].SemanticName, "NORMAL") == 0)
			{
				flag = USE_NORMAL;
			}
			else if (physx::PxStricmp(inputDesc[i].SemanticName, "TANGENT") == 0)
			{
				flag = USE_TANGENT;
			}
			else if (physx::PxStricmp(inputDesc[i].SemanticName, "COLOR") == 0)
			{
				flag = USE_COLOR;
			}
			else if (physx::PxStricmp(inputDesc[i].SemanticName, "POSITION") == 0)
			{
				flag = USE_POSITION;
			}
			...
			// Shift the sementic bit depending on the element's relative format size
