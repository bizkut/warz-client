...
#undef CASE
			...
			char buf[128];
			physx::string::sprintf_s(buf, 128,  "GL error %x occurred (%s)\n", error, errorString);
			if (m_errorCallback)
...
//				PxUserProfilerCallbackProfileEvent _pe2("cgGLSetMatrixParameter",0,__FILE__,__LINE__);
#endif
...
#ifdef PX_PS3
#define SWAP_16(x)  x = ((x & 0x00FF) << 8 ) | ((x & 0xFF00) >> 8);
#define SWAP_32(x)  x = ((x & 0x00FF) << 24 ) | ((x & 0x0FF00) << 8) | ((x & 0x0FF0000) >> 8) | ((x & 0x0FF000000) >> 24);    
		SWAP_16(bitmapFileHeader.bfType);
		SWAP_32(bitmapFileHeader.bfSize);
		SWAP_32(bitmapFileHeader.bfOffBits);
		SWAP_32(bitmapInfoHeader.bmiHeader.biSize);
		SWAP_32(bitmapInfoHeader.bmiHeader.biWidth);
		SWAP_32(bitmapInfoHeader.bmiHeader.biHeight);
		SWAP_16(bitmapInfoHeader.bmiHeader.biPlanes);
