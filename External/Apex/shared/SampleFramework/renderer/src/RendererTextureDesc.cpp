...
{
	bool ok = true;
	if(format      >= RendererTexture::NUM_FORMATS)    ok = false;
	if(filter      >= RendererTexture::NUM_FILTERS)    ok = false;
	if(addressingU >= RendererTexture::NUM_ADDRESSING) ok = false;
	if(addressingV >= RendererTexture::NUM_ADDRESSING) ok = false;
	if(width <= 0 || height <= 0 || depth <= 0) ok = false; // TODO: check for power of two.
	if(numLevels <= 0) ok = false;
	if(renderTarget)
	{
		if(depth > 1) ok = false;
		if(format == RendererTexture::FORMAT_DXT1) ok = false;
		if(format == RendererTexture::FORMAT_DXT3) ok = false;
		if(format == RendererTexture::FORMAT_DXT5) ok = false;
	}
	return ok;
}
