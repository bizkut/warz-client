...
{
	m_shaderContext.m_fogColorAndDistance = PxVec4(fogColor.r, fogColor.g, fogColor.b, fogDistance);
}
...
void GLES2Renderer::clearBuffers(void)
{
	...
	{
		glFinish();
		GLbitfield glbuf = GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT /* | 0x8000 /* GL_COVERAGE_BUFFER_BIT_NV */;
		glClear(glbuf);
	}
	end();
}
	...
	if(desc.isValid())
	{
		switch(desc.type)
		{
			case RendererLight::TYPE_DIRECTIONAL:
				light = new GLES2RendererDirectionalLight(*static_cast<const RendererDirectionalLightDesc*>(&desc), *this);
				break;
			case RendererLight::TYPE_SPOT:
				light = new GLES2RendererSpotLight(*static_cast<const RendererSpotLightDesc*>(&desc), *this);
				break;
		}
	}
	return light;
...
void GLES2Renderer::bindViewProj(const physx::PxMat44 &eye, const RendererProjection &proj)
{
		...
		m_shaderContext.m_eyePosition  =  eye.getPosition();
		m_shaderContext.m_eyeDirection = -eye.getBasis(2);
}

void GLES2Renderer::bindAmbientState(const RendererColor &ambient)
{	
	m_shaderContext.m_ambientColor = PxVec3(ambient.r / 255.0f, ambient.g / 255.0f, ambient.b / 255.0f);
}
...
void GLES2Renderer::setCommonRendererParameters() 
{
	...
	if((program.vertexMojoResult || program.fragmentMojoResult) && (g_hackCurrentMat != NULL))
	{
		const RendererMaterial::Variable* var = g_hackCurrentMat->findVariable("g_eyePosition", RendererMaterial::VARIABLE_FLOAT3);
		if(var) g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_eyePosition.x);
		var = g_hackCurrentMat->findVariable("g_eyeDirection", RendererMaterial::VARIABLE_FLOAT3);
		if(var) g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_eyeDirection.x);
		var = g_hackCurrentMat->findVariable("g_ambientColor", RendererMaterial::VARIABLE_FLOAT3);
		if(var) g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_ambientColor.x);
		var = g_hackCurrentMat->findVariable("g_fogColorAndDistance", RendererMaterial::VARIABLE_FLOAT4);
		if(var) g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_fogColorAndDistance.x);
		var = g_hackCurrentMat->findVariable("g_lightColor", RendererMaterial::VARIABLE_FLOAT3);
		if(var) g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightColor);
		var = g_hackCurrentMat->findVariable("g_lightIntensity", RendererMaterial::VARIABLE_FLOAT);
		if(var)	{ g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightIntensity);	}
		var = g_hackCurrentMat->findVariable("g_lightDirection", RendererMaterial::VARIABLE_FLOAT3);
		if(var)	g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightDirection);
		var = g_hackCurrentMat->findVariable("g_lightPosition", RendererMaterial::VARIABLE_FLOAT3);
		if(var)	g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightPosition);
		var = g_hackCurrentMat->findVariable("g_lightInnerRadius", RendererMaterial::VARIABLE_FLOAT);
		if(var) g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightInnerRadius);
		var = g_hackCurrentMat->findVariable("g_lightOuterRadius", RendererMaterial::VARIABLE_FLOAT);
		if(var)	g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightOuterRadius);
		var = g_hackCurrentMat->findVariable("g_lightInnerCone", RendererMaterial::VARIABLE_FLOAT);
		if(var)	g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightInnerCone);
		var = g_hackCurrentMat->findVariable("g_lightOuterCone", RendererMaterial::VARIABLE_FLOAT);
		if(var)	g_hackCurrentMat->bindVariable(g_hackCurrentMat->m_currentPass, *var, &m_shaderContext.m_lightOuterCone);		
	}
}
...
bool GLES2Renderer::initTexter()
{
	...
	const PxU32 MIN_CHARACTER_HEIGHT = 16;
	const PxU32 TEXT_MAX_LINES = (height / MIN_CHARACTER_HEIGHT);
	const PxU32 TEXT_MAX_VERTICES = 4 * (width / MIN_CHARACTER_WIDTH) * TEXT_MAX_LINES;
