...
{	
	...
	static const unsigned int xInputLibCount = 3;
	static const char* xInputLibs[xInputLibCount] = { "xinput1_3.dll",
	                                                  "xinput1_2.dll",
	                                                  "xinput1_1.dll" };
	...
	registerUserInput(WKEY_PRIOR ,"KEY_PRIOR","PageUp");
	registerUserInput(WKEY_NEXT ,"KEY_NEXT","PageDown");
	...
	// mouse
	registerUserInput(MOUSE_BUTTON_LEFT_DOWN ,"MOUSE_BUTTON_LEFT_DOWN","left mouse button down");
	registerUserInput(MOUSE_BUTTON_LEFT_UP ,"MOUSE_BUTTON_LEFT_UP","left mouse button up");
	registerUserInput(MOUSE_BUTTON_RIGHT_DOWN ,"MOUSE_BUTTON_RIGHT_DOWN","right mouse button down");
	registerUserInput(MOUSE_BUTTON_RIGHT_UP,"MOUSE_BUTTON_RIGHT_UP","right mouse button up");
	registerUserInput(MOUSE_BUTTON_CENTER_DOWN ,"MOUSE_BUTTON_CENTER_DOWN","center mouse button down");
	registerUserInput(MOUSE_BUTTON_CENTER_UP ,"MOUSE_BUTTON_CENTER_UP","center mouse button up");
	...
	registerUserInput(GAMEPAD_RIGHT_STICK_X,"GAMEPAD_RIGHT_STICK_X", "gpad RSTICK X");
	registerUserInput(GAMEPAD_RIGHT_STICK_Y,"GAMEPAD_RIGHT_STICK_Y", "gpad RSTICK Y");
	registerUserInput(GAMEPAD_LEFT_STICK_X,"GAMEPAD_LEFT_STICK_X", "gpad LSTICK X");
	registerUserInput(GAMEPAD_LEFT_STICK_Y,"GAMEPAD_LEFT_STICK_Y", "gpad LSTICK Y");
...
{	
	...
	if(events)
	{
		for (size_t i = events->size(); i--;)
		{
			...
			{		
				getInputEventListener()->onPointerInputEvent(ie, x, y, dx, dy);
			}
		}
	}
...
void WindowsSampleUserInput::doOnMouseButton(physx::PxU32 x, physx::PxU32 y , MouseButtons button, bool down)
{
	...
	case LEFT_MOUSE_BUTTON:
		{
			if(down)
			{
				buttonId = MOUSE_BUTTON_LEFT_DOWN;
				buttonIdOpposite = MOUSE_BUTTON_LEFT_UP;
			}
			else
			{
				buttonId = MOUSE_BUTTON_LEFT_UP;
				buttonIdOpposite = MOUSE_BUTTON_LEFT_DOWN;
			}
		}
		break;
	case RIGHT_MOUSE_BUTTON:
		{
			if(down)
			{
				buttonId = MOUSE_BUTTON_RIGHT_DOWN;
				buttonIdOpposite = MOUSE_BUTTON_RIGHT_UP;
			}
			else
			{
				buttonId = MOUSE_BUTTON_RIGHT_UP;
				buttonIdOpposite = MOUSE_BUTTON_RIGHT_DOWN;
			}
		}
		break;
	case CENTER_MOUSE_BUTTON:
		{
			if(down)
			{
				buttonId = MOUSE_BUTTON_CENTER_DOWN;
				buttonIdOpposite = MOUSE_BUTTON_CENTER_UP;
			}
			else
			{
				buttonId = MOUSE_BUTTON_CENTER_UP;
				buttonIdOpposite = MOUSE_BUTTON_CENTER_DOWN;
			}
		}
		break;
	}
	...
	if(events)
	{
		for (size_t i = events->size(); i--;)
		{
			...
			if(getInputEventListener())
			{		
				getInputEventListener()->onPointerInputEvent(ie, x, y, 0, 0);
			}
		}
	}

	events = getInputEvents((PxU16)buttonIdOpposite);
	if(events)
	{
		for (size_t i = events->size(); i--;)
		{
