...
void SampleRenderer::convertToD3D11(XMFLOAT3& dxcolor, const RendererColor& color)
{
	static const float inv255 = 1.0f / 255.0f;
	dxcolor = XMFLOAT3(color.r * inv255, color.g * inv255, color.b * inv255);
}

void SampleRenderer::convertToD3D11(XMFLOAT4& dxcolor, const RendererColor& color)
{
	static const float inv255 = 1.0f / 255.0f;
	dxcolor = XMFLOAT4(color.r * inv255, color.g * inv255, color.b * inv255, color.a * inv255);
}
...
void SampleRenderer::convertToD3D11(XMFLOAT4X4& dxmat, const RendererProjection& mat)
{
	XMMATRIX temp;
	mat.getColumnMajor44(&temp._11);
	XMStoreFloat4x4(&dxmat, XMMatrixTranspose(temp));
}

void SampleRenderer::convertToD3D11(XMMATRIX& dxmat, const physx::PxMat44 &mat)
{
	PxMat44 mat44 = mat.getTranspose();
	// this is really bad, it breaks strict aliasing rules
	//memcpy(&dxmat._11, mat44.front(), 4 * 4 * sizeof (float));
	dxmat = XMMATRIX(mat44.front());
}

void SampleRenderer::convertToD3D11(XMMATRIX& dxmat, const RendererProjection& mat)
{
	mat.getColumnMajor44(&dxmat._11);
...
void D3D11Renderer::D3D11ShaderEnvironment::bindLight(PxU32 lightIndex) const
{
	...
	if (lightIndex < RENDERER_MAX_LIGHTS)
	{
		...
		constantManager->setSharedVariable(lightName, SHADER_VAR_NAME(lightColor),       &lightColor[lightIndex].x,     sizeof(XMFLOAT3), lightIndex*sizeof(XMFLOAT3));
		constantManager->setSharedVariable(lightName, SHADER_VAR_NAME(lightDirection),   &lightDirection[lightIndex].x, sizeof(XMFLOAT3), lightIndex*sizeof(XMFLOAT3));
		constantManager->setSharedVariable(lightName, SHADER_VAR_NAME(lightPosition),    &lightPosition[lightIndex].x,  sizeof(XMFLOAT3), lightIndex*sizeof(XMFLOAT3));
...
void D3D11Renderer::D3D11ShaderEnvironment::bindBones() const
{
	if (numBones > 0)
	{
		constantManager->setSharedVariable("cbBones", SHADER_VAR_NAME(boneMatrices), &boneMatrices[0], numBones * sizeof(XMFLOAT4X4));
	}
}
...
D3D11Renderer::D3D11Renderer(const RendererDesc& desc, const char* assetDir) :
	Renderer(DRIVER_DIRECT3D11, assetDir),
	m_displayWidth(0),
	m_displayHeight(0),
	m_displayBuffer(0),
	m_vsync(desc.vsync),
	...
	m_resourceManager(NULL)
{
	...
	m_viewMatrix = physx::PxMat44::createIdentity();
...
void D3D11Renderer::bindViewProj(const physx::PxMat44& eye, const RendererProjection& proj)
{
	...
	convertToD3D11(viewMatrix, m_viewMatrix);
	convertToD3D11(projMatrix, proj);

	XMMATRIX viewProjMatrix = XMMatrixMultiply(viewMatrix, projMatrix);
	XMVECTOR invViewProjMatrixDet;
	XMMATRIX invViewProjMatrix = XMMatrixInverse(&invViewProjMatrixDet, viewProjMatrix);

	XMStoreFloat4x4(&m_environment.viewMatrix,        viewMatrix);
	XMStoreFloat4x4(&m_environment.projMatrix,        projMatrix);
	XMStoreFloat4x4(&m_environment.invViewProjMatrix, invViewProjMatrix);

	const PxVec3 eyePosition = eye.getPosition();
	const PxVec3 eyeDirection = -eye.getBasis(2);
	memcpy(&m_environment.eyePosition.x,  &eyePosition.x,  sizeof(float) * 3);
	memcpy(&m_environment.eyeDirection.x, &eyeDirection.x, sizeof(float) * 3);
...
void D3D11Renderer::renderTextBuffer(const void* vertices, PxU32 nbVerts, const PxU16* indices, PxU32 nbIndices, RendererMaterial* material)
{
	...
	if (m_d3dDeviceContext && m_d3dDevice && m_boundMaterial)
	{
		...
		if (m_textMeshes[m_currentTextMesh])
		{
			...
			if (pViewVariable)
			{
				XMMATRIX viewMatrix(2.f / m_displayWidth   , 0,  0, -1,
				                      0, -2.f / m_displayHeight, 0,  1,
				                      0                    , 0,  1,  0,
				                      0                    , 0,  0,  1);
