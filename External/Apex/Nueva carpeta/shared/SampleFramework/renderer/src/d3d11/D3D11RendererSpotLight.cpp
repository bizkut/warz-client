...
void D3D11RendererSpotLight::bind(PxU32 lightIndex) const
{
	if (lightIndex < RENDERER_MAX_LIGHTS)
	{
		...
		convertToD3D11(shaderEnv.lightPosition[lightIndex],  m_position);
		convertToD3D11(shaderEnv.lightDirection[lightIndex], m_direction);
		...
		buildProjectMatrix(&shaderEnv.lightShadowMatrix._11, m_shadowProjection, m_shadowTransform);
