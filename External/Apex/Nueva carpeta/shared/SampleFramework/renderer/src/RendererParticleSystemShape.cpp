...
RendererParticleSystemShape::RendererParticleSystemShape(Renderer &renderer, 
	PxU32 _mMaxParticles, bool _mInstanced, bool _mFading,
									PxReal fadingPeriod, PxReal debriScaleFactor) : 
...
void RendererParticleSystemShape::initializeBuffersAsSimpleConvex(PxU32 color, PxReal scaleFactor) 
{
	...
	PxU8* locked_normals = static_cast<PxU8*>(mVertexBuffer->lockSemantic(RendererVertexBuffer::SEMANTIC_NORMAL, normalStride));
	...
	PxU32 maxVertices = mVertexBuffer->getMaxVertices();
	...
	mVertexBuffer->unlockSemantic(RendererVertexBuffer::SEMANTIC_NORMAL);
...
void RendererParticleSystemShape::initializeVertexBuffer(PxU32 color) 
{
	...
	inbdesc.maxIndices = mMaxParticles;
	mIndexBuffer = m_renderer.createIndexBuffer(inbdesc);
	
	PxU16* ib = (PxU16*) mIndexBuffer->lock();
	for (PxU16 j = 0; j<mMaxParticles; j++)
	{
		ib[j] = j;
}
	mIndexBuffer->unlock();
...
void RendererParticleSystemShape::update(PxU32 validParticleRange, 
										 ...
										 const PxReal* lifetime) 
{
	...
	if(mFading)
	{
		locked_colors = static_cast<PxU8*>(mVertexBuffer->lockSemantic(
