...
public:
		...
		, mRendererMeshShape(NULL)
	{
		...
		const physx::PxU32*  indexBuffer	= (physx::PxU32*) ((mTriMesh->has16BitTriangleIndices())?NULL:mTriMesh->getTriangles());
		const physx::PxU16*  indexBuffer16	= (physx::PxU16*) ((mTriMesh->has16BitTriangleIndices())?mTriMesh->getTriangles():NULL);
		...
		else if(indexBuffer16)
		{
			for (physx::PxU32 i = 0; i < nbTris; i++)
			{
				const physx::PxVec3& A( vertices[indexBuffer16[3*i+0]] );
				const physx::PxVec3& B( vertices[indexBuffer16[3*i+1]] );
				const physx::PxVec3& C( vertices[indexBuffer16[3*i+2]] );
