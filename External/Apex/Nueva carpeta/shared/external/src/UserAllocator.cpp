...
void* UserPxAllocator::allocate(size_t size, const char* typeName, const char* filename, int line)
{
	...
	TRACKER_CALL_IF(trackerEnabled(), mMemoryTracker->trackAlloc(getThreadId(),ret, size, MEM_TRACKER::MT_MALLOC, USERALLOCATOR, typeName, filename, line));
...
{
	bool leaked = mNumFrees != mNumFrees;
