...
public:
  ...
  size_t seek(size_t loc,size_t mode)
  {
  	...
  	if ( mFph )
  	{
  		ret = fseek(mFph,(int)loc,(int)mode);
  	}
  	else
  	{
  		if ( mode == SEEK_SET )
...
//    printf("Destructed _HtmlTable(%08X) Count:%d\r\n", this, gTableCount );
  ...
  void saveCPP(FILE_INTERFACE *fph)
  {
    fi_fprintf(fph,"  if ( 1 )\r\n");
    fi_fprintf(fph,"  {\r\n");
    fi_fprintf(fph,"    physx::HtmlTable *table = document->createHtmlTable(\"%s\");\r\n", mHeading.c_str() );
    HtmlRowVector::iterator i;
    for (i=mBody.begin(); i!=mBody.end(); i++)
    {
      HtmlRow *row = (*i);
    ...
    if ( fph )
    {
      ...
      switch ( type )
      {
        ...
        case HST_CPP:
          ...
          fi_fprintf(fph,"%s\r\n","void testSave(physx::HtmlDocument *document,const char *fname,physx::HtmlSaveType type)");
          fi_fprintf(fph,"%s\r\n","{");
          fi_fprintf(fph,"%s\r\n","  size_t len;");
          ...
          fi_fprintf(fph,"{\r\n");
          fi_fprintf(fph,"  physx::HtmlTableInterface *iface = physx::getHtmlTableInterface();\r\n");
          fi_fprintf(fph,"  physx::HtmlDocument *document    = iface->createHtmlDocument(\"%s\");\r\n", mDocumentName.c_str() );
#endif
          break;
        case HST_TEXT:
        case HST_TEXT_EXTENDED:
      ...
      switch ( type )
      {
        ...
        case HST_CPP:
#if USE_CPP
          fi_fprintf(fph,"%s\r\n","  testSave(document,\"table.txt\",  physx::HST_TEXT );");
          fi_fprintf(fph,"%s\r\n","  testSave(document,\"table.html\", physx::HST_SIMPLE_HTML );");
          fi_fprintf(fph,"%s\r\n","  testSave(document,\"table.cpp\",  physx::HST_CPP );");
          fi_fprintf(fph,"%s\r\n","");
          fi_fprintf(fph,"%s\r\n","");
          fi_fprintf(fph,"%s\r\n","");
