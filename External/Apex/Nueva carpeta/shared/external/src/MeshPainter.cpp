...
{
	for (int i = 0; i < (int)mFlagBuffers.size(); i++)
...
void MeshPainter::paintFlag(unsigned int id, unsigned int flag, bool useAND) const
{
	...
	if (mBrushMode == 1)
	{
		...
		for (size_t i = 0; i < mVertices.size(); i++)
		{
			...
			unsigned int& u = mFlagBuffers[bufferNr][(int)i];
			if (useAND)
			{
				u &= flag;
			}
	...
	else if (mBrushMode == 0)
	{
		...
		for (size_t i = 0; i < mCollectedVertices.size(); i++)
		{
			float dist2 = (mLastRaycastPos - mVertices[mCollectedVertices[i]]).magnitudeSquared();
			if (dist2 > paintRadius2)
			{
			...
			unsigned int& u = mFlagBuffers[bufferNr][mCollectedVertices[i]];
			if (useAND)
			{
				u &= flag;
			}
