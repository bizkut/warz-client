...
#if defined(PX_VC10) && USE_HASH_MAP
...
{
public:
	BySourceIndex(const char *fileName,physx::PxU32 lineno)
	{
		mFileName = fileName;
		mLineNo = lineno;
	}
...
public:
  ...
  void generateReport(physx::HtmlDocument *document,physx::HtmlTable *contextTable)
  {
...

	  {
...

		{
		  ...
		  physx::HtmlTable *table = document->createHtmlTable(scratch);
		  //                    1           2            3
		  ...
		  for (ByTypeHash::iterator iter = bt.begin(); iter !=bt.end(); ++iter)
		  {
			...
			table->addColumn( (physx::PxU32)b.mSize );
			table->addColumn( (physx::PxU32)b.mCount );
			table->nextRow();
		  }
		  table->computeTotals();
		}
...

	  {
...

		{
		  ...
		  for (BySourceHash::iterator i=bt.begin(); i!=bt.end(); ++i)
		  {
			...
			table->addColumn( (physx::PxU32)b.mLineNo );
...
public:
	...
	MyMemTracker(void)
	{
	  ...
	  mFrameSummary = 0;
	  mDetailed = 0;
	  physx::HtmlTableInterface *h = physx::getHtmlTableInterface();
	  if ( h )
	  {
  ...
  virtual void trackAlloc(size_t threadId,void *mem,size_t size,MemoryType type,const char *context,const char *className,const char *fileName,physx::PxU32 lineno)
  {
	if ( mem )
	{
	  ...
	  if ( found != mMemory.end()  )
	  {
		  ...
		  printf("Prev: %s\r\n", previous.mClassName );
		  PX_FORCE_PARAMETER_REFERENCE(previous);
	  }
	  ...
	  if ( mDetailed )
	  {
		...
		mDetailed->addColumn((physx::PxU32)size);
		mDetailed->addColumn((physx::PxU32)t.mAllocCount);
		mDetailed->addColumnHex(threadId);
		mDetailed->addColumn(context);
		mDetailed->addColumn(className);
		mDetailed->addColumn(fileName);
		mDetailed->addColumn((physx::PxU32)lineno);
		mDetailed->nextRow();
	  }
  ...
  virtual const char * trackValidateFree(size_t threadId,void *mem,MemoryType type,const char *context,const char *fileName,physx::PxU32 lineno)
  {
	  const char *ret = NULL;
	  if ( mem )
	  {
		  ...
		  else
		  {
			  ...
			  case MT_DELETE:
				  if ( t.mType != MT_NEW )
				  {
					  sprintf_s(scratch,1024,"Error: Allocated with %s but deallocated the delete operator %s\r\n", typeString(t.mType));
				  }
				  break;
			  case MT_DELETE_ARRAY:
  ...
  virtual void trackFree(size_t threadId,void *mem,MemoryType type,const char *context,const char *fileName,physx::PxU32 lineno)
  {
	if ( mem )
	{
	  size_t hash = (size_t) mem;
	  ...
	  else
	  {
		  ...
		  if ( mDetailed )
		  {
			...
			mDetailed->addColumn((physx::PxU32)t.mSize);
			mDetailed->addColumn((physx::PxU32)t.mAllocCount);
			mDetailed->addColumnHex(threadId);
			mDetailed->addColumn(context);
  ...
  void trackFrame(void)
  {
	...
	if ( mAllocFrameCount || mFreeFrameCount )
	{
	  if ( mFrameSummary )
	  {
		mFrameSummary->addColumn((physx::PxU32)mFrameNo);
		mFrameSummary->addColumn((physx::PxU32)mAllocCount);
		mFrameSummary->addColumn((physx::PxU32)mAllocSize);
		mFrameSummary->addColumn((physx::PxU32)mAllocFrameCount);
		mFrameSummary->addColumn((physx::PxU32)mAllocFrameSize);
		mFrameSummary->addColumn((physx::PxU32)mFreeFrameCount);
		mFrameSummary->addColumn((physx::PxU32)mFreeFrameSize);
		mFrameSummary->addColumn((physx::PxU32)(mAllocFrameCount-mFreeFrameCount));
		mFrameSummary->addColumn((physx::PxU32)(mAllocFrameSize-mFreeFrameSize));
	  ...
	  {
		char scratch[512];
		sprintf(scratch,"New Frame %d", mFrameNo );
		mDetailed->addColumn(scratch);
		mDetailed->addColumn((physx::PxU32)mAllocSize);
		mDetailed->nextRow();
	  }
  ...
  void *generateReport(MemoryReportFormat format,const char *fname,physx::PxU32 &saveLen,bool reportAllLeaks)
  {
	  void *ret = NULL;
	  saveLen = 0;

	//
	physx::HtmlTable *contextTable = NULL;
	{
		char scratch[512];
		char date_time[512];
		getDateTime(date_time);
	...
	ReportContext *current = 0;
	  ...
	  if ( reportAllLeaks )
	  {
		...
		physx::HtmlTable *table = mDocument->createHtmlTable(scratch);
		//                    1              2           3          4       5      6            7         8
		...
		for (MemTrackHash::iterator i=mMemory.begin();  i!=mMemory.end(); ++i)
		{
			...
			table->addColumn((physx::PxU32) t.mAllocCount ); // allocation count.
			table->addColumn((physx::PxU32) t.mSize );       // size of allocation.
			...
			table->addColumn((physx::PxU32) t.mLineNo );
			table->nextRow();
		  }
		  table->computeTotals();
		}
	...
	switch ( format )
	{
		...
		case MRF_TEXT_EXTENDED:     // Saves the tables out in human readable text format, but uses the MS-DOS style extended ASCII character set for the borders.
			saveType = physx::HST_TEXT_EXTENDED;
			break;
	}
...
private:
  ...
  physx::HtmlTable    *mFrameSummary;
  physx::HtmlTable    *mDetailed;
  physx::HtmlDocument *mDocument;
