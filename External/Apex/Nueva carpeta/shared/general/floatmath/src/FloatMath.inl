...
class KdTreeNodeBundle : public UserAllocated
{
public:
...
PxU32  fm_copyUniqueVertices(PxU32 vcount,const REAL *input_vertices,REAL *output_vertices,PxU32 tcount,const PxU32 *input_indices,PxU32 *output_indices)
{
  ...
  REAL *vertices = (REAL *)PX_ALLOC(sizeof(REAL)*vcount*3, PX_DEBUG_EXP("fm_copyUniqueVertices"));
  memcpy(vertices,input_vertices,sizeof(REAL)*vcount*3);
  REAL *dest = output_vertices;

  PxU32 *reindex = (PxU32 *)PX_ALLOC(sizeof(PxU32)*vcount, PX_DEBUG_EXP("fm_copyUniqueVertices"));
...
public:
  ...
  const PxU32 * tesselate(fm_VertexIndex *vindex,PxU32 tcount,const PxU32 *indices,PxF32 longEdge,PxU32 maxDepth,PxU32 &outcount)
  {
    ...
    if ( mVertices->isDouble() )
    {
      ...
      PxF64 *vertices = (PxF64 *)PX_ALLOC(sizeof(PxF64)*vcount*3, PX_DEBUG_EXP("fm_Tesselate"));
    ...
    {
      PxU32 vcount = mVertices->getVcount();
      PxF32 *vertices = (PxF32 *)PX_ALLOC(sizeof(PxF32)*vcount*3, PX_DEBUG_EXP("fm_Tesselate"));
...
void CTriangulator::_process(TU32Vector &indices)
{
    ...
    if (n < 3)
        return;
    PxI32 *V = (PxI32 *)PX_ALLOC(sizeof(PxI32)*n, PX_DEBUG_EXP("CTriangulator"));
...
class Triangulate : public fm_Triangulate, public UserAllocated
{
public:
  Triangulate(void)
  {
                                             ...
                                             PxF64 epsilon)
  {
    ...
    PxF64 *points = (PxF64 *)PX_ALLOC(sizeof(PxF64)*pcount*3, PX_DEBUG_EXP("fm_Triangulate"));
    if ( consolidate )
    {
    ...
    if ( pcount >= 3 )
    {
      ...
      {
        tcount = _tcount;
        mPointsDouble = (PxF64 *)PX_ALLOC(sizeof(PxF64)*tcount*3*3, PX_DEBUG_EXP("fm_Triangulate"));
                                             ...
                                             PxF32 epsilon)
  {
    ...
    PxF64 *temp = (PxF64 *)PX_ALLOC(sizeof(PxF64)*pcount*3, PX_DEBUG_EXP("fm_Triangulate"));
    PxF64 *dest = temp;
    for (PxU32 i=0; i<pcount; i++)
    {
    ...
    if ( results )
    {
      ...
      mPointsFloat = (PxF32 *)PX_ALLOC(sizeof(PxF32)*tcount*3*3, PX_DEBUG_EXP("fm_Triangulate"));
      for (PxU32 i=0; i<fcount; i++)
      {
        mPointsFloat[i] = (PxF32) results[i];
      }
      PX_FREE(mPointsDouble);
      mPointsDouble = 0;
    }
