...
public:
  ...
  {
	  char scratch[512];
	  MESH_IMPORT_STRING::sprintf_s(scratch,512,"%s",fname);
...
//    fi_fprintf(fph,"       <MeshAABB min=\"%s,%s,%s\" max=\"%s,%s,%s\"/>\r\n", FloatString(a.mMin[0]), FloatString(a.mMin[1]), FloatString(a.mMin[2]), FloatString(a.mMax[0]), FloatString(a.mMax[1]), FloatString(a.mMax[2]) );
  ...
  void printVertex(FILE_INTERFACE *fph,MiU32 flags,const MeshVertex &v,MiU32 &column,bool &newRow)
  {
    ...
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mPos[0]), FloatString(v.mPos[1]), FloatString(v.mPos[2]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }

    if ( flags & MIVF_NORMAL )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mNormal[0]), FloatString(v.mNormal[1]), FloatString(v.mNormal[2]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }

    if ( flags & MIVF_COLOR )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%08X ", v.mColor );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_TEXEL1 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s ", FloatString(v.mTexel1[0]), FloatString(v.mTexel1[1]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_TEXEL2 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s ", FloatString(v.mTexel2[0]), FloatString(v.mTexel2[1]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_TEXEL3 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s ", FloatString(v.mTexel3[0]), FloatString(v.mTexel3[1]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_TEXEL4 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s ", FloatString(v.mTexel4[0]), FloatString(v.mTexel4[1]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_TANGENT )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mTangent[0]), FloatString(v.mTangent[1]), FloatString(v.mTangent[2]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_BINORMAL )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mBiNormal[0]), FloatString(v.mBiNormal[1]), FloatString(v.mBiNormal[2]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_BONE_WEIGHTING )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s %s ", FloatString(v.mWeight[0]), FloatString(v.mWeight[1]), FloatString(v.mWeight[2]), FloatString(v.mWeight[3]) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%d %d %d %d ", v.mBone[0], v.mBone[1], v.mBone[2], v.mBone[3] );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP1 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp1[0]), FloatString(v.mInterp1[1]), FloatString(v.mInterp1[2]) , FloatString(v.mInterp1[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP2 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp2[0]), FloatString(v.mInterp2[1]), FloatString(v.mInterp2[2]) , FloatString(v.mInterp2[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP3 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp3[0]), FloatString(v.mInterp3[1]), FloatString(v.mInterp3[2]) , FloatString(v.mInterp3[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP4 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp4[0]), FloatString(v.mInterp4[1]), FloatString(v.mInterp4[2]) , FloatString(v.mInterp4[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP5 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp5[0]), FloatString(v.mInterp5[1]), FloatString(v.mInterp5[2]) , FloatString(v.mInterp5[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP6 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp6[0]), FloatString(v.mInterp6[1]), FloatString(v.mInterp6[2]) , FloatString(v.mInterp6[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP7 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp7[0]), FloatString(v.mInterp7[1]), FloatString(v.mInterp7[2]) , FloatString(v.mInterp7[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_INTERP8 )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s %s %s ", FloatString(v.mInterp8[0]), FloatString(v.mInterp8[1]), FloatString(v.mInterp8[2]) , FloatString(v.mInterp8[3]));
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    if ( flags & MIVF_RADIUS )
    {
      MESH_IMPORT_STRING::sprintf_s(temp,1024,"%s ", FloatString(v.mRadius) );
      MESH_IMPORT_STRING::strcat_s(scratch,1024,temp);
    }
    MESH_IMPORT_STRING::strcat_s(scratch,1024,",    ");
    MiU32 slen = (MiI32)strlen(scratch);
    ...
    void printIndex(FILE_INTERFACE *fph,const MiU32 *idx,MiU32 &column,bool &newRow)
  {
    ...
    char temp[1024];
    MESH_IMPORT_STRING::sprintf_s(temp,1024,"%d %d %d,  ", idx[0], idx[1], idx[2] );
...
//		  MiI32 mBoneIndex;
      ...
      if ( ms->mAnimationCount )
      {
        ...
        MESH_IMPORT_STRING::strcpy_s(ainfo.mGroup,64,"NONE");
  ...
  void serializeOgre(FILE_INTERFACE *fph,FILE_INTERFACE *exfph,MeshSystem *mesh,const char *saveName)
  {
    ...
    if ( mesh->mSkeletonCount )
    {
      ...
      if ( saveName )
      {
        ...
        char scratch[512];
        MESH_IMPORT_STRING::strcpy_s(scratch,512,slash);
  ...
  typedef STDNAME::vector< MiU32 > NxU32Vector;
  ...
  void serializeWavefront(FILE_INTERFACE *fph,FILE_INTERFACE *exfph,MeshSystem *mesh,const char *saveName,const MiF32 *exportTransform)
  {
    ...
    char scratch[512];
    MESH_IMPORT_STRING::strcpy_s(scratch,512,saveName);
    char *dot = (char *)lastDot(scratch);
    if ( dot )
    {
      *dot = 0;
    }
    MESH_IMPORT_STRING::strcat_s(scratch,512,".mtl");
  ...
  virtual const char   *    getFileRequestDialogString(void)
  {
    ...
    MiU32 count = getImporterCount();
    for (unsigned i=0; i<count; i++)
    {
      mimp::MeshImporter *imp = getImporter(i);
      MiU32 ecount = imp->getExtensionCount();
  ...
  virtual MeshSkeletonInstance *createMeshSkeletonInstance(const MeshSkeleton &sk)
  {
    ...
    if ( sk.mBoneCount )
    {
      ...
      ret->mBones = new MeshBoneInstance[sk.mBoneCount];
