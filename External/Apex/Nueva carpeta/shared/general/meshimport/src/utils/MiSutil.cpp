...
***/
	...
	else
	{
		...
		{
			MiU32 *iv = (MiU32 *) &v;
			MESH_IMPORT_STRING::sprintf_s(ret,64,"%.4f$%x", v, *iv );
		}
		else
		{
			MESH_IMPORT_STRING::sprintf_s(ret,64,"%.9f", v );
...
MiI32 GetUserArgs(const char *us,const char *key,const char **args)
{
	...
	char keyword[512];
	MESH_IMPORT_STRING::sprintf_s(keyword,512,"%s(", key );
...
const char	*fstring(MiF32 v)
{
	...
	{
		MESH_IMPORT_STRING::sprintf_s(ret,16, "%.9f", v);
		const	char *dot	=	strstr(ret,	".");
		if (dot)
		{
			MiI32	len	=	(MiI32)strlen(ret);
...
#if defined (LINUX_GENERIC) || defined(LINUX) || defined(__CELLOS_LV2__) || defined(__APPLE__) || defined(ANDROID)
