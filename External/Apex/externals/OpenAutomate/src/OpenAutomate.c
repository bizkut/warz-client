...
#elif __CELLOS_LV2__ || SN_TARGET_PSP2 || PX_MOCHA
typedef size_t OA_NATIVE_INT;
#elif _XBOX
typedef size_t OA_NATIVE_INT;
#else
# include <dlfcn.h>
...
static void (*dummy_oaiInitFuncTable)(oaiFunctionTable *table);
static int (*dummy_oaiPluginInitVersionStruct)(oaVersion *plugin_version, oaVersion app_version);
...
oaBool oaInit(const oaChar *init_str, oaVersion *version)
{
  ...
  dummy_oaiInitFuncTable = oaiInitFuncTable; // suppress unused static warning
  dummy_oaiPluginInitVersionStruct = oaiPluginInitVersionStruct; // suppress unused static warning
     ...
     PluginVersion.Minor < OA_MIN_REQUIRED_VERSION_MINOR))
  {
    ...
    sprintf(ErrorStr, 
            ...
            (int) PluginVersion.Major,
            (int) PluginVersion.Minor);
...
#elif defined(__CELLOS_LV2__) || defined(SN_TARGET_PSP2)
	/* NOP for now */
#elif defined(_XBOX)
	/* NOP for now */
#elif defined(ANDROID)
    /* FIXME!! it has a runtime bug here which cannot call dlclose. */
#else
...
#if OA_PLATFORM == OA_LINUX
  InitStr = SearchAndReplace(init_str, "%3B", ";");
#endif  
  int i=0;
  for(i=0; InitStr[i]; ++i)
  {
...
static oaChar* SearchAndReplace(const oaChar* src, oaChar* search, oaChar* replace)
{
  ...
  while ((Src2 = strstr (Src1, search)))
  {
    Num = (OA_NATIVE_INT) (size_t) (Src2 - Src1);
  ...
  OA_STRCPY_S(Dest, ((NewLen+1)*sizeof(oaChar)), Src1);

  return ReturnStr;
}
#if defined(_XBOX) || defined(__CELLOS_LV2__) || defined(SN_TARGET_PSP2)
#ifdef __cplusplus
extern "C"
{
#endif
extern oaiFunctionTable *oaPluginInit(const char *init_str, 
                                         oaVersion *version,
...
#elif OA_PLATFORM == OA_MOCHA || OA_PLATFORM == OA_PS3 || OA_PLATFORM == OA_PSP2
  /* NOP for now */
  PluginHandle = NULL;
#elif OA_PLATFORM == OA_XBOX
  PluginHandle = NULL;
#else
...
#elif OA_PLATFORM == OA_PS3 || OA_PLATFORM == OA_PSP2
	oaPluginInit;
#elif OA_PLATFORM == OA_XBOX
	oaPluginInit;
#else
