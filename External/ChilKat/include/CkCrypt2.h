...
class CkCrypt2  : public CkMultiByteBase
{
	...
	void SetSecretKey(const char *pByteData, unsigned long szByteData);
	void SetIV(const char *pByteData, unsigned long szByteData);
	...
	bool SetEncryptCert(const CkCert &cert) { return this->SetEncryptCert(&cert); }
	bool SetSigningCert(const CkCert &cert) { return this->SetSigningCert(&cert); }
	bool SetVerifyCert(const CkCert &cert) { return this->SetVerifyCert(&cert); }
	...
	void put_Salt(const CkByteData &data);
	// SALT_END
