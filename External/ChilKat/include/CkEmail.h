...
class CkEmail : public CkMultiByteBase
{
	...
	void AddRelatedData2P(const char *pByteData, unsigned long szByteData, const char *fileNameInHtml);
	bool AddRelatedDataP(const char *nameInHtml, const char *pByteData, unsigned long szByteData, CkString *strContentId);
	bool AddRelatedDataP(const char *nameInHtml, const unsigned char *pByteData, unsigned long szByteData, CkString *strContentId);

	bool AddDataAttachment(const char *fileName, const char *pByteData, unsigned long szByteData);
    	bool SetMbPlainTextBody(const char *charset, const char *pByteData, unsigned long szByteData);
    	bool SetMbHtmlBody(const char *charset, const char *pByteData, unsigned long szByteData);
	bool QEncodeBytes(const char *pByteData, unsigned long szByteData, const char *charset, CkString &outEncodedStr);
	bool BEncodeBytes(const char *pByteData, unsigned long szByteData, const char *charset, CkString &outEncodedStr);
	const char *qEncodeBytes(const char *pByteData, unsigned long szByteData, const char *charset);
	const char *bEncodeBytes(const char *pByteData, unsigned long szByteData, const char *charset);
	bool UidlEquals(const CkEmail *e);

	bool AddFileAttachment(const char *path, CkString *strContentType);
    	bool AddRelatedFile(const char *path, CkString *strContentID);
	...
	bool SetFromMimeText2(const char *pByteData, int szByteData);
	...
	bool GetRelatedStringCrLf(long index, const char *charset, CkString &outStr);
	...
	bool GetRelatedString(long index, const char *charset, CkString &outStrData);
	...
	bool AddRelatedFile(const char *path, CkString &outStrContentID) { return AddRelatedFile(path,&outStrContentID); }
...
#if !defined(CHILKAT_MONO)
	bool AddRelatedDataP(const char *nameInHtml, const unsigned char *pByteData, unsigned long szByteData, CkString &outStrContentId){return AddRelatedDataP(nameInHtml,pByteData,szByteData,&outStrContentId);}
#endif
	...
	bool GetAttachmentStringCrLf(long index, const char *charset, CkString &outStrData);
	...
	bool GetAttachmentHeader(long index, const char *fieldName, CkString &outFieldValue);
	...
	bool GetAttachmentString(long index, const char *charset, CkString &outStr);
	...
	bool AddFileAttachment(const char *path, CkString &outStrContentType){return AddFileAttachment(path,&outStrContentType);}
	...
	//bool AddBinaryAttachment(const char *fileName, CkByteData &content);
	...
	bool AddRelatedString(const char *nameInHtml, const char *str, const char *charset, CkString &outCid){return AddRelatedString(nameInHtml,str,charset,&outCid);}
	...
	*/
	...
	// display it. If inPath is empty, a temporary filename is generated and this filename is returned.
	// If inPath is non-empty, then it is created or overwritten, and the inPath 
	// is simply returned in outPath.
	...
	// GETREPORT_END
	// GETALTERNATIVEBODYBYCONTENTTYPE_BEGIN
	...
	// GETALTERNATIVEBODYBYCONTENTTYPE_END
	...
	// ADDICALENDARALTERNATIVEBODY_BEGIN
	bool AddiCalendarAlternativeBody(const char *body, const char *methodName);
	// ADDICALENDARALTERNATIVEBODY_END
	// ADDRELATEDDATA_BEGIN
