...
{
    SF_ASSERT(Locked != 0); 
    LockedBuffers.UnlockAll(pDeviceContext);
    Locked = false;
    if (pRQCaches)
...
handle_alloc_fail:
    ...
    pvdata = pvb->Lock(LockedBuffers, pDeviceContext);
    pidata = pib->Lock(LockedBuffers, pDeviceContext);
