...
namespace Cm
{
	...
	class RefResolver : public PxRefResolver
	{
		public:
		...
		virtual	void			setStringTable(const char* stringTable)
		{
			mStringTable = stringTable;
		}

		virtual	const char*		resolveName(const char* name)
		{
	...
	class UserReferences : public PxUserReferences, public Ps::UserAllocated
	{
		...
		virtual void*			internal_getObjectFromRef(InternalUserRef internalRef) const;
		virtual bool			internal_setObjectRef(void* object, InternalUserRef internalRef);
	...
	*/
	...
	PxUserReferences* createUserReferences(const void * refs, PxU32 count);

} // namespace Cm

}

PX_C_EXPORT PX_PHYSX_COMMON_API physx::PxUserReferences* PX_CALL_CONV PxCreateUserReferences(const void * refs = 0, physx::PxU32 count = 0);
