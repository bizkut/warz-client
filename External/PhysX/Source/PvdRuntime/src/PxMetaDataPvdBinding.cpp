...
inline void definePropertyFlags( PvdDataStream& inStream, const char* inPropertyName )
{
	...
	inStream.createProperty<TDataType,PxU32>( inPropertyName,"Bitflag", PropertyType::Scalar, helper.getNamedValues());
...
inline void definePropertyEnums( PvdDataStream& inStream, const char* inPropertyName )
{
	...
	inStream.createProperty<TDataType,PxU32>( inPropertyName,"Enumeration Value", PropertyType::Scalar, helper.getNamedValues());
	helper.clearNamedValues();
}

namespace {

PX_FORCE_INLINE void registerPvdRaycast( PvdDataStream& inStream )
{
...
PX_FORCE_INLINE void registerPvdSweep( PvdDataStream& inStream )
{
...
PX_FORCE_INLINE void registerPvdOverlap( PvdDataStream& inStream )
{
...
PX_FORCE_INLINE void registerPvdSqHit( PvdDataStream& inStream )
{
...
template <typename TTargetType, PxU32 T_NUM_ITEMS, typename TSourceType = TTargetType, typename Converter = NullConverter<TTargetType> >
class ScopedPropertyValueSender
{
	TTargetType			stack[T_NUM_ITEMS];
	TTargetType*			mCur;
...
public:
	...
	~ScopedPropertyValueSender() 
	{
		if ( stack != mCur )
		{
			PxU32 size = sizeof (TTargetType) * PxU32( mCur - stack );
			mStream.appendPropertyValueData( DataRef<const PxU8>( (PxU8*)stack, size ) );
		}
	...
	void append( const TSourceType& data )
	{
		...
		{
			mStream.appendPropertyValueData( DataRef<const PxU8>( (PxU8*)stack, sizeof stack ) );
			mCur = stack;
		}
	}
};
...
{
	void operator()( PvdContact& data, const Sc::ContactIterator::Contact& src )
	{
...
{
	ScopedPropertyValueSender<PvdContact, 32, Sc::ContactIterator::Contact, PvdContactConverter> sender( inStream, &inScene, "Contacts" );
...
void PvdMetaDataBinding::sendSceneQueries( PvdDataStream& inStream, const PxScene& inScene, bool hasValue )
{
	if(!hasValue)
		return;
...
#define SEND_PVD_GEOM_TYPE( enumType, TGeomType, TValueType )											\
			case enumType:																				\
				{																						\
					...
					GeometryBufferRegisterOp<TGeomType>().registerBuffers( inObj, sdkPvd );				\
