...
//----------------------------------------------------------------------------------------------------------------------
template<typename T>
class NMTL_TEMPLATE_EXPORT stack
{
public:
  typedef stack<T> my_type;
  ...
  /// \brief  ctor
  NMTL_INLINE stack(nmtl::allocator* allocator_ = NMTL_DEFAULT_ALLOCATOR) :
  ...
  NMTL_INLINE stack(const stack<T>& s) :
    m_container(s.m_container)
  {
  }

  /// \brief  construct by copying specified container
  /// \param  _Cont - the container to copy
  NMTL_INLINE explicit stack(const container_type& container) :
