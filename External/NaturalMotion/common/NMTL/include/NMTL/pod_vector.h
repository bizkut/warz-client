...
//----------------------------------------------------------------------------------------------------------------------
#if (!defined(NMTL_POD_VECTOR_H) && !defined(NMTL_NO_SENTRY)) || defined(NMTL_NO_SENTRY)
#ifndef NMTL_NO_SENTRY
  #define NMTL_POD_VECTOR_H
#endif
...
//----------------------------------------------------------------------------------------------------------------------
/// \class   NMTL::vector
/// \brief   defines a templated array container
/// \ingroup NaturalMotionTemplateLibrary
//----------------------------------------------------------------------------------------------------------------------
template<typename T>
class NMTL_TEMPLATE_EXPORT pod_vector
{
public:
  //--------------------------------------------------------------------------------------
  // vector typedefs
  ...
  NMTL_INLINE pod_vector(nmtl::allocator* allocator_ = NMTL_DEFAULT_ALLOCATOR) :
    m_first(0), m_last(0), m_end(0), m_allocator(allocator_)
  {
  }
  ...
  NMTL_INLINE pod_vector(
    size_type count, const T& val = T(), nmtl::allocator* allocator_ = NMTL_DEFAULT_ALLOCATOR) :
    m_first(0), m_last(0), m_end(0), m_allocator(allocator_)
  {
    resize(count, val);
  }
  ...
  NMTL_INLINE pod_vector(const my_type& right) :
    m_first(0), m_last(0), m_end(0), m_allocator(right.m_allocator)
  {
    reserve(right.size());
    m_end = m_last;
  ...
  NMTL_INLINE pod_vector(
    const_iterator start, const_iterator finish, nmtl::allocator* allocator_ = NMTL_DEFAULT_ALLOCATOR) :
    m_first(0), m_last(0), m_end(0), m_allocator(allocator_)
  {
    assign(start, finish);
  }
    ...
    if (capacity() < count)
    {
      ...
      if (temp && m_first)
      {
        for (size_t s = 0; s < old_size; ++s)
        {
          ...
          // Using a tempory variable for the ptr arithmentic fixes x64 compile warning C4328:
          // 'indirection alignment of formal parameter parameter_number (alignment_value_for_parameter)
          // is greater than the actual argument alignment (actual_alignment)'
          pointer t = temp + s;
          create(t, m_first[s]);
        }
      }
  ...
  NMTL_INLINE void resize(size_type new_size)
  {
    ...
    if (new_size > old_size)
    {
      ...
      // Using a tempory variable for the ptr arithmentic fixes x64 compile warning C
      pointer temp1 = m_first + old_size;
      pointer temp2 = m_first + new_size;
      create(temp1, temp2);
    }
  ...
  /// \brief  set an element at the specified index in the array
  /// \param  pos - the index of the element to set
  /// \param  val - the value to use
  NMTL_INLINE void set(NMTL_INDEX size_type pos, const T& val)
  {
    reference element = at(pos);
    element = val;
  }
  ...
  NMTL_INLINE iterator insert(iterator location, const_iterator start, const_iterator finish)
  {
    ...
    {
      create((location + i).m_ptr, start[i]);
    }
  ...
  NMTL_INLINE iterator insert(iterator location, size_type count, const T& val)
  {
    ...
    pointer first_plus_w0 = m_first + w0;
    location = first_plus_w0;
  ...
  NMTL_INLINE void copy(const_iterator first, const_iterator last, iterator dst)
  {
    ...
    memcpy(dst.m_ptr, first.m_ptr, (last.m_ptr - first.m_ptr) * sizeof(value_type));
  }

  /// \brief  clears all elements in the array, but does not change capacity
  ...
  /// \brief returns the memory usage of the vector
  NMTL_INLINE size_t getMemoryUsage() const
  {
    return sizeof(pod_vector<value_type>) + sizeof(value_type) * capacity();
  }
...
NMTL_INLINE bool operator ==(const pod_vector<value_type>& lhs, const pod_vector<value_type>& rhs)
{
  ...
  typename pod_vector<value_type>::const_iterator ita = lhs.begin();
  typename pod_vector<value_type>::const_iterator itb = rhs.begin();
