...
//----------------------------------------------------------------------------------------------------------------------
  ...
  #pragma once
#endif
#ifndef NM_NONLINEAR_OPTIMISER_H
#define NM_NONLINEAR_OPTIMISER_H
...
//----------------------------------------------------------------------------------------------------------------------
/// \brief This class represents a standard Jacobian matrix (full sized and non-sparse).
template <typename _T>
class Jacobian : public JacobianBase<_T>
{
public:
  Jacobian(uint32_t numRows, uint32_t numColumns);
  ~Jacobian();
...
protected:
  NMP::Matrix<_T> m_matrix;
};

//----------------------------------------------------------------------------------------------------------------------
/// \brief This class represents a standard Hessian matrix J^T * J (full sized and non-sparse).
template <typename _T>
class Hessian : public HessianBase<_T>
{
public:
  Hessian(uint32_t size);
  ~Hessian();
...
/// approximation.
template <typename _T>
class NonLinearOptimiser : public NonLinearOptimiserBase<_T>
{
public:
  //---------------------------------------------------------------------
  /// \name   Constructors
  ...
  //---------------------------------------------------------------------
  //@{
  NonLinearOptimiser(NonLinearLeastSquaresFunc<_T>& func);
  NonLinearOptimiser(NonLinearLeastSquaresFuncJac<_T>& funcJac);
...
protected:
  // Symmetric Eigen-system to solve A x = b
  NMP::EigenSystemSym<_T> m_solver;
...
#endif // NM_NONLINEAR_OPTIMISER_H
