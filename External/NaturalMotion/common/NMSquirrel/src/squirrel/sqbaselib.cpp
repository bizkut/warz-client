...
static SQInteger base_dummy(HSQUIRRELVM SQUIRREL_UNUSED(v))
{
  return 0;
}
...
static SQInteger base_getstackinfos(HSQUIRRELVM v)
{
  ...
  if (SQ_SUCCEEDED(sq_stackinfos(v, level, &si)))
  {
    ...
    sq_newtable(v);
    seq=0;
    while ((name = sq_getlocal(v, level, seq)) != 0) {
...
//QSORT ala Sedgewick
bool _qsort_compare(HSQUIRRELVM v,SQObjectPtr& SQUIRREL_UNUSED(arr),SQObjectPtr &a,SQObjectPtr &b,SQInteger func,SQInteger &ret)
{
  if(func < 0) {
...
{ \
  ...
  for(SQInteger i=0;i<len;i++) sNew[i]=static_cast<SQChar>(func(sThis[i])); \
