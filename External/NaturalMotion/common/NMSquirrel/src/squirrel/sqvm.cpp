...
#pragma warning(push)
#endif // _MSC_VER
    // conversion from 'const SQRawObjectVal' to 'void *' of greater size
#ifdef _MSC_VER
#pragma warning(disable: 4312)
#endif // _MSC_VER
    const size_t size = (sizeof(void*) * 2) + 17;
    scsprintf(_sp(rsl(size)), size, _SC("(%s : 0x%p)"),GetTypeName(o),reinterpret_cast<void*>(_rawval(o)));
#ifdef _MSC_VER
#pragma warning(pop)
#endif // _MSC_VER
  }
...
bool SQVM::FOREACH_OP(SQObjectPtr &o1,SQObjectPtr &o2,SQObjectPtr 
                      &o3,SQObjectPtr &o4,SQInteger SQUIRREL_UNUSED(arg_2),int exitpos,int &jump)
{
  SQInteger nrefidx;
  switch(type(o1)) {
  case OT_TABLE:
  ...
  //
  {
    for(;;)
    {
      ...
      case _OP_CALL: {
        ...
        switch (type(clo)) {
      case OT_CLOSURE:
        _GUARD(StartCall(_closure(clo), arg0, arg3, _stackbase+arg2, false));
        continue;
...
bool SQVM::Get(const SQObjectPtr &self,const SQObjectPtr &key,SQObjectPtr &dest,bool raw, SQInteger selfidx)
{
  ...
  case OT_TABLE:
    ...
    break;
  case OT_ARRAY:
    if(sq_isnumeric(key)) { return _array(self)->Get(tointeger(key),dest); }
    break;
  case OT_INSTANCE:
...
void SQVM::FindOuter(SQObjectPtr &target, SQObjectPtr *stackindex)
{
  ...
  otr->_next = *pp;
  otr->_idx  = static_cast<SQInteger>(stackindex - _stack._vals);
  __ObjAddRef(otr);
  *pp = otr;
