...
static SQInteger sqstd_rex_element(SQRex *exp)
{
  ...
  case '{':{
    ...
    p0 = static_cast<unsigned short>(sqstd_rex_parsenumber(exp));
    switch(*exp->_p) {
  case '}':
    p1 = p0; exp->_p++;
    goto __end;
  case ',':
    ...
    p1 = 0xFFFF;
    if(isdigit(*exp->_p)){
      p1 = static_cast<unsigned short>(sqstd_rex_parsenumber(exp));
    }
    sqstd_rex_expect(exp,'}');
    goto __end;
  default:
...
case OP_OR: {
  ...
  while ( (asd = sqstd_rex_matchnode(exp,temp,asd)) != 0) 
  {
    if(temp->next != -1)
...
case OP_NOCAPEXPR:{
  ...
  if(capture != -1) 
    exp->_matches[capture].len = static_cast<SQInteger>(cur - exp->_matches[capture].begin);
  return cur;
                  }				 
case OP_WB:
