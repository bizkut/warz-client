...
#ifdef _MSC_VER
// conversion from 'unsigned int' to 'SQUserPointer' of greater size
...
//basic API
SQFILE sqstd_fopen(HSQUIRRELVM v,const SQChar *filename,const SQChar *mode)
{
  (void)v;
...
#else
  char mode_thin[10];
  wcstombs(mode_thin,mode,10);
  size_t length = wcslen(filename) + 1;
  char* filename_thin = (char*)sq_getscratchpad(v, (SQInteger)length);
  wcstombs(filename_thin,filename,length);
  return (SQFILE)fopen(filename_thin,mode_thin);
#endif
#ifdef _MSC_VER
#pragma warning(pop)
#endif
...
struct SQFile : public SQStream {
  ...
  bool Open(HSQUIRRELVM v,const SQChar *filename,const SQChar *mode) {
    Close();
    if( (_handle = sqstd_fopen(v,filename,mode)) ) {
      _owns = true;
      return true;
    }
    return false;
...
static SQInteger _file_releasehook(SQUserPointer p, SQInteger SQUIRREL_UNUSED(size))
{
...
static SQInteger _file_constructor(HSQUIRRELVM v)
{
  ...
  if(sq_gettype(v,2) == OT_STRING && sq_gettype(v,3) == OT_STRING) {
    ...
    newf = sqstd_fopen(v,filename, mode);
...
{
  char c;
  SQInteger ret = sqstd_fread(&c,sizeof(c),1,(FILE *)file );
  if ( ret > 0 )
    return c;
  return 0;
}
...
{
  SQFILE file = sqstd_fopen(v,filename,_SC("rb"));
  SQInteger ret;
  unsigned short us;
  unsigned char uc;
  ...
  if(file){
    ...
    if(us == SQ_BYTECODE_STREAM_TAG) { //BYTECODE
      ...
      case 0xBBEF: 
        ...
        if(uc != 0xBF) { 
          sqstd_fclose(file); 
          return sq_throwerror(v,_SC("Unrecognozed encoding")); 
        }
...
{
  SQFILE file = sqstd_fopen(v,filename,_SC("wb+"));
