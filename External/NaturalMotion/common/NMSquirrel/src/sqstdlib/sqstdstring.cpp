...
#  define scstrtok strtok_s
# else
#  define scstrtok(SQ_TOKEN_ARG,SQ_DELIMIT_ARG,SQ_CONTEXT_ARG) strtok(SQ_TOKEN_ARG,SQ_DELIMIT_ARG)
# endif
#endif
...
static SQInteger validate_format(HSQUIRRELVM v, SQChar *fmt, const SQChar *src, SQInteger n,SQInteger &width)
{
  ...
  if(wc > 0) {
    SQChar* endptr;
    width = scstrtol(swidth,&endptr,10);
  }
  else
    width = 0;
  if (src[n] == '.') {
...
SQRESULT sqstd_format(HSQUIRRELVM v,SQInteger nformatstringidx,SQInteger *outlen,SQChar **output)
{
  ...
  while(format[n] != '\0') {
    if(format[n] != '%') {
      ...
      const SQChar *ts = 0;
      SQInteger ti = 0;
      SQFloat tf = static_cast<SQFloat>(0.0);
      switch(format[n]) {
      case 's':
      ...
      dest = sq_getscratchpad(v,allocated);
      switch(valtype) {
      case 's': i += scsprintf(&dest[i],allocated - i,fmt,ts); break;
      case 'i': i += scsprintf(&dest[i],allocated - i,fmt,ti); break;
      case 'f': i += scsprintf(&dest[i],allocated - i,fmt,tf); break;
      };
      nparam ++;
    }
  }
...
static SQInteger _string_strip(HSQUIRRELVM v)
{
  ...
  sq_pushstring(v,start,static_cast<SQInteger>(end - start));
  return 1;
}
...
static SQInteger _string_rstrip(HSQUIRRELVM v)
{
  ...
  SQInteger len = sq_getsize(v,2);
  __strip_r(str,len,&end);
  sq_pushstring(v,str,static_cast<SQInteger>(end - str));
  return 1;
}
...
#endif
  SQChar* endptr = 0; (void)endptr;
  tok = scstrtok(stemp,seps,&endptr);
#ifdef _MSC_VER
#pragma warning(pop)
#endif
  sq_newarray(v,0);
...
static SQInteger _rexobj_releasehook(SQUserPointer p, SQInteger SQUIRREL_UNUSED(size))
{
  SQRex *self = ((SQRex *)p);
  sqstd_rex_free(self);
  return 1;
...
static void _addrexmatch(HSQUIRRELVM v,const SQChar *str,const SQChar *begin,const SQChar *end)
{
  ...
  sq_pushinteger(v,static_cast<SQInteger>(begin - str));
  sq_rawset(v,-3);
  sq_pushstring(v,_SC("end"),-1);
  sq_pushinteger(v,static_cast<SQInteger>(end - str));
  sq_rawset(v,-3);
}
