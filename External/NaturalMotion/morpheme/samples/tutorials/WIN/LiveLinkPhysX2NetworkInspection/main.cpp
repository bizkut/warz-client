...
int32_t networkLiveLinkInspectionWithPhysics(
  ...
  Game::CharacterManagerPhysX2& characterManager)
{
  //--------------------------------------------------------------------------------------------------------------------
  ...
  Game::CharacterPhysX2* gameCharacter = characterManager.createCharacter(gameCharacterDef);
  if (!gameCharacter)
  {
...
int32_t main(int32_t NMP_UNUSED(argc), char** NMP_UNUSED(argv))
{
  ...
  physXManager.init();
  NxScene* nxScene = physXManager.createScene();
  if (!nxScene)
  {
  ...
  //----------------------------
  // Game::CharacterManagerPhysX2 is a static class which will act as a manager for all the morpheme network definitions 
  ...
  //----------------------------
  // When we are ready to exit we can terminate Game::CharacterManagerPhysX2 to free any classes/memory it has stored.
  // Since Game::CharacterManagerPhysX2 stores a pointer to the CharacterDef and Character instances it will manage
