...
//----------------------------------------------------------------------------------------------------------------------
int32_t networkLiveLinkInspectionWithEuphoria(
  Game::EuphoriaManager&          euphoriaManager,
  Game::CharacterManagerEuphoria& characterManager)
{
  ...
  LiveLinkEuphoriaDataManager customEuphoriaDataManager(&networkManager); 
  ...
  Game::CharacterDefEuphoria* characterEuphoriaDef = (Game::CharacterDefEuphoria*) characterManager.getCharacterDef(0);
  networkManager.publishNetworkDef(characterEuphoriaDef->getNetworkDef(), characterEuphoriaDef->getNetworkDefGuid());

  NMP_STDOUT("\nCreating GameCharacter from GameCharacterDef:");
  Game::CharacterEuphoria* gameCharacter = characterManager.createCharacter(characterEuphoriaDef);
  if (!gameCharacter)
  {
  ...
  MCOMMS::RuntimeTargetInterface runtimeTarget(
    ...
    &customEuphoriaDataManager, //< EuphoriaDataManagementInterface
  ...
  // Add all known modules and debug controls to the debug interface
  gameCharacter->initDebugInterfaceModuleNamesAndControls();
  ER::networkGetCharacter(gameCharacter->getNetwork())->setDebugInterface(&debugInterface);
  ...
  MCOMMS::EuphoriaCommsServerModule* const euphoriaCommsServerModule = new MCOMMS::EuphoriaCommsServerModule();
  ...
  while (!connectionManager.connectionsReducedToZero())
  {
    ...
    commsServer.beginFrame(DELTA_TIME);
    {
      ...
      euphoriaManager.simulate(DELTA_TIME);
...
int32_t main(int32_t NMP_UNUSED(argc), char** NMP_UNUSED(argv))
{
  ...
  euphoriaManager.init();
  physx::PxScene* physXScene = euphoriaManager.createScene();
  if (!physXScene)
  {
  ...
  euphoriaManager.connectToPhysXVisualDebugger();

  //---------------------------
  // Create a simple physical environment, in this case a plane the character can stand on.
  physx::PxRigidActor* physXGroundObject = euphoriaManager.createPhysicsObjectGroundPlane();
  ...
  characterManager.attachToPhysXScene(
                      euphoriaManager.getPhysXScene(),
                      euphoriaManager.getPhysXCharacterControllerManager());
  ...
  Game::CharacterDefEuphoria* gameCharacterDef = characterManager.createCharacterDef(NETWORK_DEF_BUNDLE_NAME);
  if (!gameCharacterDef)
  {
  ...
  euphoriaMessageID = gameCharacterDef->getNetworkDef()->getMessageIDFromMessageName(EUPHORIA_NAME);
  ...
  const int32_t result = networkLiveLinkInspectionWithEuphoria(euphoriaManager, characterManager);
  ...
  euphoriaManager.destroyPhysicsObject(physXGroundObject);
  euphoriaManager.destroyScene();
  euphoriaManager.term();
