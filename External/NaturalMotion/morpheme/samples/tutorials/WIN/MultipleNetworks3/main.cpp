...
#define NETWORK_DEF_BUNDLE_NAME1 ASSETLOCATION "/Network_MultipleNetworks3.nmb"
#define NETWORK_DEF_BUNDLE_NAME2 ASSETLOCATION "/Network2_MultipleNetworks3.nmb"
...
int32_t main(int32_t NMP_UNUSED(argc), char** NMP_UNUSED(argv))
{
  ...
  // NETWORKPATH and NETWORKPATH2 and use that information to apply to the character definition instances.
  ...
  //----------------------------
  // And repeat the process for the second character definition.
  ...
  NMP_STDOUT("\nCreating Character1 from CharacterDef1:"); 
  Game::CharacterNoPhysics* gameCharacter1 = characterManager.createCharacter(gameCharacterDef1);

  NMP_STDOUT("\nCreating Character2 from CharacterDef1:");
  Game::CharacterNoPhysics* gameCharacter2 = characterManager.createCharacter(gameCharacterDef1);

  NMP_STDOUT("\nCreating Character3 from CharacterDef2:");
  Game::CharacterNoPhysics* gameCharacter3 = characterManager.createCharacter(gameCharacterDef2);

  NMP_STDOUT("\nCreating Character4 from CharacterDef2:");
  Game::CharacterNoPhysics* gameCharacter4 = characterManager.createCharacter(gameCharacterDef2);
  ...
  if(!gameCharacter3)
  {
    NMP_STDOUT("\nError: Failed to create Character 3");
    return NULL;
  }

  if(!gameCharacter4)
  {
    NMP_STDOUT("\nError: Failed to create Character 4");
    return NULL;
  }
  ...
  //----------------------------
  // Move character 1 and 3 slightly so that we know it is working correctly
  NMP::Vector3 setPosition1 = NMP::Vector3(9.0f, 0.0f, -3.0f);
  NMP::Vector3 setPosition3 = NMP::Vector3(-9.0f, 0.0f, 3.0f);
  ...
  Game::CharacterControllerNoPhysics* characterController3 = static_cast<Game::CharacterControllerNoPhysics*>(gameCharacter3->getNetwork()->getCharacterController());
  Game::CharacterControllerNoPhysics* characterController4 = static_cast<Game::CharacterControllerNoPhysics*>(gameCharacter4->getNetwork()->getCharacterController());

  if(!characterController1 || !characterController2 || !characterController3 || !characterController4)
  {
    NMP_STDOUT("\n Error: Failed to find character controllers.  Have they been initialised correctly?");
    return NULL;
  }

  characterController1->setPosition(setPosition1);
  characterController3->setPosition(setPosition3);
  ...
  const NMP::Vector3 position3 = characterController3->getPosition();
  const NMP::Vector3 position4 = characterController4->getPosition();
  ...
  NMP_STDOUT("End Position of gameCharacter3: %.3fx, %.3fy, %.3fz", position3.x, position3.y, position3.z);
  NMP_STDOUT("End Position of gameCharacter4: %.3fx, %.3fy, %.3fz", position4.x, position4.y, position4.z);
