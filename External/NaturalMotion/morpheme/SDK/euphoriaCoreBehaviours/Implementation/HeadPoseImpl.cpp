...
//----------------------------------------------------------------------------------------------------------------------
void HeadPoseUpdatePackage::update(float NMP_UNUSED(timeStep), MR::InstanceDebugInterface* MR_OUTPUT_DEBUG_ARG(pDebugDrawInst))
{
  float poseHIImp = in->getHeadPoseHighImpImportance() ;
  if (poseHIImp > 0.0f)
  {
    PoseData poseData = in->getHeadPoseHighImp();
    // Set control limb.
    ...
    TM.multiply(owner->owner->data->headLimbSharedStates[owner->data->childIndex].m_rootTM);
    NMP::Vector3 vel
      = owner->owner->data->headLimbSharedStates[owner->data->childIndex].m_rootTM.getRotatedVector(
  ...
  if (poseHIImp < 1.0f && poseLIImp > 0.0f)
  {
    ...
    NMP::Vector3 vel 
      = owner->owner->data->headLimbSharedStates[owner->data->childIndex].m_rootTM.getRotatedVector(
      poseData.poseEndRelativeToRootVelocity);
    NMP::Vector3 angVel = owner->owner->data->headLimbSharedStates[owner->data->childIndex].m_rootTM.getRotatedVector(
      poseData.poseEndRelativeToRootAngularVelocity);
    control.setTargetPos(TM.translation(), 1.0f, vel);
    control.setTargetOrientation(TM, 1.0f, angVel);
    control.setGravityCompensation(poseData.gravityCompensation);
