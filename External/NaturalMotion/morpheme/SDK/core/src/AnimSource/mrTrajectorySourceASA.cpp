...
//----------------------------------------------------------------------------------------------------------------------
// TrajectorySourceASA functions.
//----------------------------------------------------------------------------------------------------------------------
#ifndef NM_HOST_CELL_SPU
void TrajectorySourceASA::locate()
{
  // Verify alignment.
  NMP_ASSERT_MSG(
  ...
  // Step over TrajectorySourceASA class instance.
  uint8_t* data = (uint8_t*) this;
  data += NMP::Memory::align(sizeof(TrajectorySourceASA), NMP_VECTOR_ALIGNMENT);

  // Delta trajectory channel set
  m_deltaTrajectoryChannelSet.locate(&data);

  m_getTrajAtTime = TrajectorySourceASA::computeTrajectoryTransformAtTime;
}

//----------------------------------------------------------------------------------------------------------------------
void TrajectorySourceASA::dislocate()
{
  // Delta trajectory channel set.
...
//----------------------------------------------------------------------------------------------------------------------
void TrajectorySourceASA::computeTrajectoryTransformAtTime(
  ...
  NMP::Vector3&               resultPos)
{
  ...
  float     interpolant;
  uint32_t  frameIndex;
  TrajectorySourceASA* trajControl = (TrajectorySourceASA*)trajectoryControl;
