  ...
  Network* net)
{
  ...
  AttribDataBool* outputCP = outputCPPin->getAttribData<AttribDataBool>();
 
  // Find an active input path and if there is one update it to get the result.
  AttribDataBool* inputCP = updateEmittedControlParamInputCPConnection<AttribDataBool>(node, net);
  if (inputCP)
  {
...
//----------------------------------------------------------------------------------------------------------------------
void nodeControlParamBoolInitInstance(NodeDef* node, Network* net)
{
  NMP_ASSERT(node->getNodeTypeID() == NODE_TYPE_CP_BOOL);
  ...
  AttribDataBool* defAttribData = node->getAttribData<AttribDataBool>(ATTRIB_SEMANTIC_CP_BOOL);
