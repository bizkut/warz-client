...
//----------------------------------------------------------------------------------------------------------------------
AttribData* nodeOperatorVector3DistanceOutputCPUpdate(
  NodeDef* node,
  ...
  Network* net)
{
  ...
  // First CP.
  AttribDataVector3* inputCPVector3A =
    net->updateInputCPConnection<AttribDataVector3>(node->getInputCPConnection(0), animSet);
  // Second CP.
  AttribDataVector3* inputCPVector3B =
    net->updateInputCPConnection<AttribDataVector3>(node->getInputCPConnection(1), animSet);
  ...
  resultOutput->m_value = NMP::vDistanceBetween(inputCPVector3A->m_value, inputCPVector3B->m_value);
