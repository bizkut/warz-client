...
//----------------------------------------------------------------------------------------------------------------------
Task* nodeHeadLookQueueHeadLookSetup(
  NodeDef*        node,
  TaskQueue*      queue,
  Network*        net,
  TaskParameter*  dependentParameter)
{
  ...
  Task* task = queue->createNewTaskOnQueue(
                 MR::CoreTaskIDs::MR_TASKID_HEADLOOKSETUP,
                 node->getNodeID(),
                 2,
                 dependentParameter,
                 false,
...
//----------------------------------------------------------------------------------------------------------------------
bool nodeHeadLookIKShouldQueuePassThrough(NodeDef* node, Network* net)
{
  const CPConnection* cpConnection = node->getInputCPConnection(1);

  AttribDataFloat* blendWeight = 0;
  ...
  if (cpConnection->m_sourceNodeID != INVALID_NODE_ID)
  {
    ...
    blendWeight = (AttribDataFloat*)outputCPPin->getAttribData();  
...
//----------------------------------------------------------------------------------------------------------------------
Task* nodeHeadLookQueueHeadLookTransformBuffs(
  NodeDef*        node,
  TaskQueue*      queue,
  Network*        net,
  TaskParameter*  dependentParameter)
{
  ...
  if (nodeHeadLookIKShouldQueuePassThrough(node, net))
  {
    return queuePassThroughChild0(node, queue, net, dependentParameter);
  }

  Task* task = queue->createNewTaskOnQueue(
                 CoreTaskIDs::MR_TASKID_HEADLOOKTRANSFORMS,
                 node->getNodeID(),
                 9,
                 dependentParameter);
#ifdef NODE_TASKS_QUEUE_BY_SEMANTICS
...
//----------------------------------------------------------------------------------------------------------------------
Task* nodeHeadLookQueueHeadLookTrajectoryDeltaAndTransformBuffs(
  NodeDef*        node,
  TaskQueue*      queue,
  Network*        net,
  TaskParameter*  dependentParameter)
{
  ...
  Task* task = queue->createNewTaskOnQueue(
    CoreTaskIDs::MR_TASKID_HEADLOOKTRAJECTORY_DELTA_AND_TRANSFORMS,
    node->getNodeID(),
    8,
    dependentParameter);
#ifdef NODE_TASKS_QUEUE_BY_SEMANTICS
...
#endif
  {
    ...
    net->TaskAddDefInputParam(
      task, 3, ATTRIB_SEMANTIC_NODE_SPECIFIC_DEF_ANIM_SET, node->getNodeID(), activeAnimSetIndex);
    ...
    // The character controller, for determining the world-space location of the character
    net->TaskAddNetInputParam(task, 7, ATTRIB_SEMANTIC_CHARACTER_PROPERTIES, NETWORK_NODE_ID, INVALID_NODE_ID, currFrameNo);
  }
