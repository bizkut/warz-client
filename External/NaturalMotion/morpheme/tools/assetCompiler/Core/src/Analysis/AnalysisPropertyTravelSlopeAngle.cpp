...
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format AnalysisPropertyTravelSlopeAngleBuilder::getMemoryRequirements(
  ...
  uint32_t                      numFrames)
{
  ...
  // Header
  NMP::Memory::Format result(sizeof(AnalysisPropertyTravelSlopeAngle), NMP_VECTOR_ALIGNMENT);
...
//----------------------------------------------------------------------------------------------------------------------
AnalysisProperty* AnalysisPropertyTravelSlopeAngleBuilder::init(
  ...
  uint32_t                      numFrames)
{
  ...
  // Header
  NMP::Memory::Format memReqsHdr(sizeof(AnalysisPropertyTravelSlopeAngle), NMP_VECTOR_ALIGNMENT);
  AnalysisPropertyTravelSlopeAngle* result = (AnalysisPropertyTravelSlopeAngle*)memRes.alignAndIncrement(memReqsHdr);
...
//----------------------------------------------------------------------------------------------------------------------
void AnalysisPropertyTravelSlopeAngleBuilder::instanceInit(  
  AnalysisProperty* property,
  MR::Network* network)
{
  NMP_VERIFY(property);
  AnalysisPropertyTravelSlopeAngle* target = (AnalysisPropertyTravelSlopeAngle*)property;
...
//----------------------------------------------------------------------------------------------------------------------
void AnalysisPropertyTravelSlopeAngleBuilder::instanceUpdate(  
  AnalysisProperty* property,
  MR::Network* network)
{
  NMP_VERIFY(property);
...
//----------------------------------------------------------------------------------------------------------------------
void AnalysisPropertyTravelSlopeAngleBuilder::evaluateProperty(AnalysisProperty* property)
{
  NMP_VERIFY(property);
  AnalysisPropertyTravelSlopeAngle* target = (AnalysisPropertyTravelSlopeAngle*)property;
  AnalysisPropertySampleBuffer* sampleBuffer = target->m_sampleBuffer;
  ...
  for (uint32_t i = 0; i < numSamples; ++i)
  {
    NMP::Vector3 deltaPos = deltaPosVec[i];
    float vertDeltaPos = deltaPos.dot(planeNormal);
    deltaPos -= (planeNormal * vertDeltaPos);
    float travelInPlane = deltaPos.magnitude();

    // Slope angle in the travel direction
    if (travelInPlane > 1e-4f)
    {
      featureData[i] = atan(vertDeltaPos / travelInPlane);
    }
    else
    {
      featureData[i] = 0.0f;
...
//----------------------------------------------------------------------------------------------------------------------
bool AnalysisPropertyTravelSlopeAngleBuilder::calculateRangeDistribution(
  AssetProcessor*             NMP_UNUSED(processor),
  uint32_t                    numSamples,
  const float*                samples,
  float&                      minVal,
  ...
  float&                      centreVal)
{
  return processHalfAngularRangeDistribution(
    numSamples,
    samples,
    minVal,
    maxVal,
