...
//----------------------------------------------------------------------------------------------------------------------
bool NodeControlParamIntBuilder::hasSemanticLookupTable()
{
  return true;
}

//----------------------------------------------------------------------------------------------------------------------
MR::SemanticLookupTable* NodeControlParamIntBuilder::initSemanticLookupTable(NMP::Memory::Resource& memRes)
{
  MR::SemanticLookupTable* semanticLookupTable = MR::SemanticLookupTable::init(memRes, NODE_TYPE_CP_INT);
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamIntBuilder::getNodeDefInputConnections(
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamIntBuilder::preInit(
...
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format NodeControlParamIntBuilder::getNodeDefMemoryRequirements(
...
//----------------------------------------------------------------------------------------------------------------------
MR::NodeDef* NodeControlParamIntBuilder::init(
...
{
  NMP_VERIFY_MSG(nodeDefExport->getTypeID() == NODE_TYPE_CP_INT, "Expecting node type CP_INT");
  const ME::DataBlockExport* nodeDefDatablock = nodeDefExport->getDataBlock();
  ...
  nodeDefDatablock->readInt(defaultInt, "DefaultInt");
  MR::AttribDataInt* defaultValueAttrib = MR::AttribDataInt::init(memRes, defaultInt, MR::IS_DEF_ATTRIB_DATA);
  initAttribEntry(
    nodeDef,
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamIntBuilder::initTaskQueuingFns(
  MR::NodeDef*                nodeDef,
  NetworkDefCompilationInfo*  netDefCompilationInfo,
  NMP::BasicLogger*           logger)
{
  ...
  if (nodeDef->getNodeFlags().areSet(MR::NodeDef::NODE_FLAG_IS_OPERATOR_NODE)) // Are we an Emitted control param.
  {
    ...
    nodeDef->setOutputCPTaskFnId(outputCPTaskFns, 0, MR::ATTRIB_SEMANTIC_CP_INT, FN_NAME(nodeControlParamIntEmittedCPUpdateInt), logger);
  ...
  nodeDef->setInitNodeInstanceFnId(FN_NAME(nodeControlParamIntInitInstance), logger);
}
