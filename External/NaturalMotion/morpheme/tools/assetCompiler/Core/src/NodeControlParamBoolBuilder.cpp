...
//----------------------------------------------------------------------------------------------------------------------
bool NodeControlParamBoolBuilder::hasSemanticLookupTable()
{
  return true;
}

//----------------------------------------------------------------------------------------------------------------------
MR::SemanticLookupTable* NodeControlParamBoolBuilder::initSemanticLookupTable(NMP::Memory::Resource& memRes)
{
  MR::SemanticLookupTable* semanticLookupTable = MR::SemanticLookupTable::init(memRes, NODE_TYPE_CP_BOOL);
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamBoolBuilder::getNodeDefInputConnections(
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamBoolBuilder::preInit(
...
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format NodeControlParamBoolBuilder::getNodeDefMemoryRequirements(
...
//----------------------------------------------------------------------------------------------------------------------
MR::NodeDef* NodeControlParamBoolBuilder::init(
...
{
  NMP_VERIFY_MSG(nodeDefExport->getTypeID() == NODE_TYPE_CP_BOOL, "Expecting node type CP_BOOL");
  const ME::DataBlockExport* nodeDefDatablock = nodeDefExport->getDataBlock();
  ...
  nodeDefDatablock->readBool(defaultFlag, "DefaultFlag");
  MR::AttribDataBool* defaultValueAttrib = MR::AttribDataBool::init(memRes, defaultFlag, MR::IS_DEF_ATTRIB_DATA);
  initAttribEntry(
    nodeDef,
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamBoolBuilder::initTaskQueuingFns(
  MR::NodeDef*                nodeDef,
  NetworkDefCompilationInfo*  netDefCompilationInfo,
  NMP::BasicLogger*           logger)
{
  ...
  if (nodeDef->getNodeFlags().areSet(MR::NodeDef::NODE_FLAG_IS_OPERATOR_NODE)) // Are we an Emitted control param.
  {
    ...
    nodeDef->setOutputCPTaskFnId(outputCPTaskFns, 0, MR::ATTRIB_SEMANTIC_CP_BOOL, FN_NAME(nodeControlParamBoolEmittedCPUpdateBool), logger);
  ...
  nodeDef->setInitNodeInstanceFnId(FN_NAME(nodeControlParamBoolInitInstance), logger);
}
