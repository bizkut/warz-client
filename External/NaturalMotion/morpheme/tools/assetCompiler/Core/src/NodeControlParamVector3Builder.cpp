...
//----------------------------------------------------------------------------------------------------------------------
bool NodeControlParamVector3Builder::hasSemanticLookupTable()
{
  return true;
}

//----------------------------------------------------------------------------------------------------------------------
MR::SemanticLookupTable* NodeControlParamVector3Builder::initSemanticLookupTable(NMP::Memory::Resource& memRes)
{
  MR::SemanticLookupTable* semanticLookupTable = MR::SemanticLookupTable::init(memRes, NODE_TYPE_CP_VECTOR3);
  NMP_ASSERT(semanticLookupTable);

  semanticLookupTable->addLookupIndex(MR::ATTRIB_SEMANTIC_CP_VECTOR3);

  return semanticLookupTable;
}

//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamVector3Builder::getNodeDefInputConnections(
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamVector3Builder::preInit(
...
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format NodeControlParamVector3Builder::getNodeDefMemoryRequirements(
...
//----------------------------------------------------------------------------------------------------------------------
MR::NodeDef* NodeControlParamVector3Builder::init(
...
{
  NMP_VERIFY_MSG(nodeDefExport->getTypeID() == NODE_TYPE_CP_VECTOR3, "Expecting node type CP_VECTOR3");
  const ME::DataBlockExport* nodeDefDatablock = nodeDefExport->getDataBlock();
  ...
  //---------------------------
  // Add attributes.
  NMP::Vector3 defaultValue(0, 0, 0);
  ...
  MR::AttribDataVector3* defaultValueAttrib = MR::AttribDataVector3::init(memRes, defaultValue, MR::IS_DEF_ATTRIB_DATA);
  initAttribEntry(
    nodeDef,
    MR::ATTRIB_SEMANTIC_CP_VECTOR3,                   // semantic,    
    0,                                                // animSetIndex,
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamVector3Builder::initTaskQueuingFns(
  MR::NodeDef*                nodeDef,
  NetworkDefCompilationInfo*  netDefCompilationInfo,
  NMP::BasicLogger*           logger)
{
  ...
  if (nodeDef->getNodeFlags().areSet(MR::NodeDef::NODE_FLAG_IS_OPERATOR_NODE)) // Are we an Emitted control param.
  {
    ...
    nodeDef->setOutputCPTaskFnId(outputCPTaskFns, 0, MR::ATTRIB_SEMANTIC_CP_VECTOR3, FN_NAME(nodeControlParamVector3EmittedCPUpdateVector3), logger);
  ...
  nodeDef->setInitNodeInstanceFnId(FN_NAME(nodeControlParamVector3InitInstance), logger);
}
