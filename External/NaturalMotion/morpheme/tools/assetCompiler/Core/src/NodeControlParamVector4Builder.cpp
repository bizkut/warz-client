...
//----------------------------------------------------------------------------------------------------------------------
bool NodeControlParamVector4Builder::hasSemanticLookupTable()
{
  return true;
}

//----------------------------------------------------------------------------------------------------------------------
MR::SemanticLookupTable* NodeControlParamVector4Builder::initSemanticLookupTable(NMP::Memory::Resource& memRes)
{
  MR::SemanticLookupTable* semanticLookupTable = MR::SemanticLookupTable::init(memRes, NODE_TYPE_CP_VECTOR4);
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamVector4Builder::getNodeDefInputConnections(
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamVector4Builder::preInit(
...
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format NodeControlParamVector4Builder::getNodeDefMemoryRequirements(
  ...
  AssetProcessor*             NMP_UNUSED(processor))
{
  ...
  result += MR::AttribDataVector4::getMemoryRequirements();

  return result;
}

//----------------------------------------------------------------------------------------------------------------------
MR::NodeDef* NodeControlParamVector4Builder::init(
...
{
  NMP_VERIFY_MSG(nodeDefExport->getTypeID() == NODE_TYPE_CP_VECTOR4, "Expecting node type CP_VECTOR4");
  const ME::DataBlockExport* nodeDefDatablock = nodeDefExport->getDataBlock();
  ...
  //---------------------------
  // Add attributes.
  NMP::Quat defaultValue(0, 0, 0, 0);
  nodeDefDatablock->readFloat(defaultValue.x, "DefaultValue_0");
  nodeDefDatablock->readFloat(defaultValue.y, "DefaultValue_1");
  nodeDefDatablock->readFloat(defaultValue.z, "DefaultValue_2");
  nodeDefDatablock->readFloat(defaultValue.w, "DefaultValue_3");
  MR::AttribDataVector4* defaultValueAttrib = MR::AttribDataVector4::init(memRes, defaultValue, MR::IS_DEF_ATTRIB_DATA);
  initAttribEntry(
    nodeDef,
...
//----------------------------------------------------------------------------------------------------------------------
void NodeControlParamVector4Builder::initTaskQueuingFns(
  MR::NodeDef*                nodeDef,
  NetworkDefCompilationInfo*  netDefCompilationInfo,
  NMP::BasicLogger*           logger)
{
  ...
  if (nodeDef->getNodeFlags().areSet(MR::NodeDef::NODE_FLAG_IS_OPERATOR_NODE)) // Are we an Emitted control param.
  {
    ...
    nodeDef->setOutputCPTaskFnId(outputCPTaskFns, 0, MR::ATTRIB_SEMANTIC_CP_VECTOR4, FN_NAME(nodeControlParamVector4EmittedCPUpdateVector4), logger);
  ...
  nodeDef->setInitNodeInstanceFnId(FN_NAME(nodeControlParamVector4InitInstance), logger);
}
