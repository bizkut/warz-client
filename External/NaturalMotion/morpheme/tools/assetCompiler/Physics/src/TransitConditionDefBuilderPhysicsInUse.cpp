...
//----------------------------------------------------------------------------------------------------------------------
NMP::Memory::Format TransitConditionDefBuilderPhysicsInUse::getMemoryRequirements(
  const ME::ConditionExport*   NMP_UNUSED(condExport),
  const ME::NetworkDefExport*  NMP_UNUSED(netDefExport),
  AssetProcessor*              NMP_UNUSED(processor))
{
  return NMP::Memory::Format(NMP::Memory::align(sizeof(MR::TransitConditionDefPhysicsInUse), NMP_NATURAL_TYPE_ALIGNMENT), NMP_NATURAL_TYPE_ALIGNMENT);
}

//----------------------------------------------------------------------------------------------------------------------
/// \brief Initialise the TransitCondition from connects xml data.
MR::TransitConditionDef* TransitConditionDefBuilderPhysicsInUse::init(
  ...
  MR::NodeID                   NMP_UNUSED(sourceNodeID))
{
  ...
  MR::TransitConditionDefPhysicsInUse* result = (MR::TransitConditionDefPhysicsInUse*) memRes.ptr;
  memRes.increment(memReqs);
  result->setType(TRANSCOND_PHYSICS_IN_USE_ID);
  result->setInvertFlag(false); // TODO: Implement UI.


  // Initialize control param attrib address to look for.
  bool onInUse;
  condDataBlock->readBool(onInUse, "OnPhysicsInUse");
  result->setOnInUse(onInUse);
