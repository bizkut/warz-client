...
//----------------------------------------------------------------------------------------------------------------------
// PhysX3Manager functions.
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::init(
#ifdef NM_HOST_CELL_PPU
  ...
  float physicsToleranceScale)
{
  ...
  m_defaultPhysXMaterial = m_physXSDK->createMaterial(
                                            g_physXDefaultMaterialStaticFriction,
                                            g_physXDefaultMaterialDynamicFriction,
                                            g_physXDefaultMaterialRestitution);
  NMP_ASSERT(m_defaultPhysXMaterial);
  m_defaultPhysXMaterial->setFrictionCombineMode(g_physXDefaultMaterialFrictionCombineMode);    // How to combine the friction of two contacting objects.
  m_defaultPhysXMaterial->setRestitutionCombineMode(g_physXDefaultMaterialRestitutionCombineMode); // How to combine the restitution of two impacting objects.
  NMP_ASSERT(m_defaultPhysXMaterial);
  ...
  // Create a default material to use for character controllers.
  m_defaultPhysXCharacterControllerMaterial = m_physXSDK->createMaterial(0.0f, 0.0f, 0.0f);
  NMP_ASSERT(m_defaultPhysXCharacterControllerMaterial);
  m_defaultPhysXCharacterControllerMaterial->setFrictionCombineMode(g_physXDefaultMaterialFrictionCombineMode);
  m_defaultPhysXCharacterControllerMaterial->setRestitutionCombineMode(g_physXDefaultMaterialRestitutionCombineMode);
}

//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::term()
{ 
  ...
  if (m_defaultPhysXCharacterControllerMaterial)
  {
    m_defaultPhysXCharacterControllerMaterial->release();
  ...
  if (m_defaultPhysXMaterial)
  {
    m_defaultPhysXMaterial->release();
...
//----------------------------------------------------------------------------------------------------------------------
bool PhysX3Manager::connectToPhysXVisualDebugger()
{
...
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::setPhysXVisualDebuggerFileName(const char* filename)
{
  if (filename)
  {
...
//----------------------------------------------------------------------------------------------------------------------
physx::PxScene* PhysX3Manager::createScene(const NMP::Vector3* gravity)
{
  // We must have initialised the PhysX SDK and we can only have one scene per instance of PhysX3Manager.
...
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::destroyScene()
{
...
//----------------------------------------------------------------------------------------------------------------------
physx::PxRigidActor* PhysX3Manager::createPhysicsObject(
  ...
  float                       angularDamping)
{
  ...
  //-----------------------
  // create a material if we were not provided with one.
  if(!material)
  {
    material = m_physXSDK->createMaterial(staticFriction, dynamicFriction, restitution);
...
//----------------------------------------------------------------------------------------------------------------------
physx::PxRigidActor* PhysX3Manager::createPhysicsObjectStaticBox(
  const NMP::Vector3& dimensions,
  const NMP::Vector3& position,
  const NMP::Quat*    rotation)
{
  // Make sure that the SDK and scene are initialised.
  NMP_ASSERT(m_physXFoundation && m_physXSDK && m_physXScene && m_defaultPhysXMaterial);
...
//----------------------------------------------------------------------------------------------------------------------
physx::PxRigidActor* PhysX3Manager::createPhysicsObjectGroundPlane(
  float               groundHeight,
  float               planeWidth,
...
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::destroyPhysicsObject(physx::PxRigidActor *actor)
{
  // Make sure that the SDK and scene are initialised.
...
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::simulate(float deltaTime)
{
  // Make sure that the SDK and scene are initialised.
...
//----------------------------------------------------------------------------------------------------------------------
bool PhysX3Manager::validatePluginList(const NMP::OrderedStringTable& pluginList)
{
...
//----------------------------------------------------------------------------------------------------------------------
// PhysX3Manager::ErrorStream functions.
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::ErrorStream::reportError(physx::PxErrorCode::Enum e, const char* message, const char* file, int line)
{
...
//----------------------------------------------------------------------------------------------------------------------
void PhysX3Manager::ErrorStream::print(const char* message)
{
  NMP_STDOUT("%s", message);
}

//----------------------------------------------------------------------------------------------------------------------
// PhysX3Manager::PhysXAllocator functions.
//----------------------------------------------------------------------------------------------------------------------
#ifdef WIN32
  ...
  // allocation fns
  void* PhysX3Manager::PhysXAllocator::allocate(size_t size, const char*, const char*, int)
  {
    return _aligned_malloc(size, 16);
  }

  //----------------------------------------------------------------------------------------------------------------------
  void PhysX3Manager::PhysXAllocator::deallocate(void* ptr)
  {
    _aligned_free(ptr);
  }
