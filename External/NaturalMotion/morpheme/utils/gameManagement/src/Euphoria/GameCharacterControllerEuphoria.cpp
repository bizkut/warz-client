...
void CharacterControllerPhysX3::addStickToGroundMovement()
{
  ...
  if (m_onGround)
  {
    ...
    float horDist = m_requestedHorizontalDeltaTranslation.magnitude();
    NMP_ASSERT(getPhysicsScene());
    NMP::Vector3 worldUpDirection = getPhysicsScene()->getWorldUpDirection();
    m_verticalStepVector =
        worldUpDirection * (-horDist * tanf(NMP::degreesToRadians(m_activeControllerDef->getMaxSlopeAngle())));
  ...
  const NMP::Quat*    requestedDeltaWorldOrientation)
{
  ...
  {
    //----------------------------
    // The character root position is being controlled by a combination of a requested delta movement and the Nx-controller semi-physics.
