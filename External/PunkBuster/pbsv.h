...
//
inline char *ltoa ( long i , char *a , int radix )
{
...
public:
	...
	inline char *getBasePath ( char *path ) {
		...
		if ( !(*path) ) _getcwd ( path , PB_Q_MAXRESULTLEN - 4 ) ;
	...
	inline char *getHomePath ( void ) {
		...
		if ( !(*m_cwd) ) _getcwd ( m_cwd , PB_Q_MAXRESULTLEN - 4 ) ;
	...
	inline char *makefn ( char *buf , char *fn ) {	//assumes buf is large enough to hold cwd + fn + overhead
		if ( !(*m_cwd) ) {
			...
			if ( _stricmp ( basepath , m_cwd ) && *basepath && *m_cwd ) {
#ifdef __PBWIN32__
				_mkdir ( m_cwd ) ;
#endif
#ifdef __PBLINUX__
	...
	inline char *LoadServerDll ( void ) {
		...
		FARPROC pProc;
		pProc = GetProcAddress ( (HINSTANCE) m_SvInstance , "sa" ) ;
		m_ProcessPbEvents = (tdPbProcessPbEvents)pProc;
		pProc = GetProcAddress ( (HINSTANCE) m_SvInstance , "sb" ) ;
		m_AddPbEvent = 	(tdPbAddSvEvent)pProc;
#endif
#ifdef __PBLINUX__
