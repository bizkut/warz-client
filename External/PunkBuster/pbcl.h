...
//note:		m_GlQuery is set in pbsdk.h
	...
	inline char *LoadClientDll ( void ) {
		...
		FARPROC pProc;
		pProc = GetProcAddress ( (HINSTANCE) m_ClInstance , "ca" ) ;
		m_ProcessPbEvents = (tdPbProcessPbEvents)pProc;
		pProc = GetProcAddress ( (HINSTANCE) m_ClInstance , "cb" ) ;
		m_AddPbEvent = 	(tdPbAddClEvent)pProc ;
#endif
#ifdef __PBLINUX__
