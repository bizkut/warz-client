...
static double randomVectors[256 * 4] =
{
...
double GradientNoise(double fx, double fy, double fz, int ix, int iy, int iz, int seed)
{
...
double GradientCoherentNoise (double x, double y, double z, int seed)
{
	...
	double xs = Curve5 (x - (double)x0);
	double ys = Curve5 (y - (double)y0);
	double zs = Curve5 (z - (double)z0);
...
{
	float normalizedValue = (float)GradientCoherentNoise(time, 0.0, 0.0, 678) * 0.5f + 0.5f;
	return normalizedValue * maxForce;
}
...
, offset( 0, 0, 0 )
{
	r3dTexture* tex = r3dRenderer->LoadTexture( "Data/Shaders/TEXTURE/wind_noise.dds" ) ;

	r3d_assert( tex->GetD3DFormat() == D3DFMT_L8 ) ;

	void* bits = tex->Lock( 0 ) ;

	windMap.Resize( tex->GetWidth(), tex->GetHeight() ) ;

	for( int y = 0, e = tex->GetHeight() ; y < e ; y ++ )
	{
		for( int x = 0, w = tex->GetWidth() ; x < w ; x ++ )
		{
			windMap[ y ][ x ] = ((UINT8*)bits)[ x + y * w ] ;
		}
	}
...
r3dWind::~r3dWind()
{
	r3dRenderer->DeleteTexture( windTex ) ;
}

const r3dWind::Settings& 
r3dWind::GetSettings() const
{
	return settings ;
}
...
void
r3dWind::SetSettings( const Settings& sts )
{
	settings = sts ;

	float len = sqrtf( settings.DirX * settings.DirX + settings.DirZ * settings.DirZ ) ;
...
void r3dWind::Load(const char* levelDir)
{
	...
	if ( script.OpenFile( FName ) )
	{
		...
		while ( ! script.EndOfFile() )
		{
			...
			else if ( ! strcmp( buffer, "Velocity" ) )
			{
...
void r3dWind::SetAzimuth(float a)
{
	
}

void r3dWind::Update( float t )
{
	float dt = t - time ;
	time = t;

	offset.x += dt * settings.WindPatternSpeed ;
	offset.z += dt * settings.WindPatternSpeed ;
}

r3dPoint3D
r3dWind::GetWindDir() const
{
	return r3dPoint3D( settings.DirX, 0.f, settings.DirZ ) ;
}

r3dTexture*
r3dWind::GetWindTexture()
{
	return windTex ;
}

D3DXVECTOR4
r3dWind::GetTexcXfm() const
{
	float s = settings.WindPatternScale ;

	return D3DXVECTOR4( 1.0f / s, 1.0f / s, offset.x / s, offset.z / s ) ;
}

UINT8
r3dWind::GetWindAt( const r3dPoint3D& pos )
{
	if( settings.ForceWind )
	{
		return R3D_MIN( UINT8( settings.ForceWindVal * 255 ), UINT8( 255 ) ) ;
	}
	else
	{
		float s = settings.WindPatternScale ;

		float x = ( pos.x + offset.x ) / s ;
		float z = ( pos.z + offset.z ) / s ;
		...
		UINT8 val00 = windMap[ iz0 ][ ix0 ] ;
		UINT8 val01 = windMap[ iz0 ][ ix1 ] ;
		UINT8 val10 = windMap[ iz1 ][ ix0 ] ;
		UINT8 val11 = windMap[ iz1 ][ ix1 ] ;

		return 
			
			(UINT8)R3D_LERP(
						R3D_LERP( (float)val00, (float)val01, dx ),
						R3D_LERP( (float)val10, (float)val11, dx ),
						dz
					) ;
	}
...
r3dWind::Settings::Settings()
: DirX( 0.f )
, DirZ( 1.f )
, WindPatternScale( 256.f )
, WindPatternSpeed( 4.f )
, ForceWind( 0 )
, ForceWindVal( 1.0f )
{
