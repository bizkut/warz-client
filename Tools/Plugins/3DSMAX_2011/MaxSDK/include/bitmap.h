...
//@}
   ...
   public:
		...
		// deprecated method - call GetPathEx(). Note that GetPathEx() returns a Path, not a reference
		// to a Path. So do not assign return value to a Path& variable.
		MAX_DEPRECATED MaxSDK::Util::Path& GetPath() const;
		...
		*/
		...
		//! \deprecated see : BMMExport void           EnumAuxFiles                ( AssetEnumCallback& assetEnum, DWORD flags);
