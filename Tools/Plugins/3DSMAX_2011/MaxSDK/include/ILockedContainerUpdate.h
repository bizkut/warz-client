...
public:
	...
	virtual bool PreReplacement(INode *sourceNode, Animatable *sourceAnim, INode *masterNode, Animatable *masterAnim, IContainerUpdateReplacedNode *man, TSTR &log)   = 0;
	...
	//! \param[in,out] log A string to place any pertinent error messages that the container update system may output as an error log during the update process.
	//! \return Returns true if the function succeeds, false otherwise.
	virtual bool PostReplacement(INode *sourceNode, Animatable *sourceAnim, INode *masterNode, IContainerUpdateReplacedNode *man, TSTR &log) = 0;
};
