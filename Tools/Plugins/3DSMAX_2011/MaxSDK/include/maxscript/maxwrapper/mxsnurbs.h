...
public:
	...
	ScripterExport ~NURBSIndependentPointValue();
...
public:
	...
	ScripterExport ~NURBSControlVertexValue();

				classof_methods(NURBSControlVertexValue, NURBSObjectValue);
	void		collect() { delete this; }
	ScripterExport void		sprin1(CharStream* s);
#	define		is_nurbscontrolvertex(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(NURBSControlVertexValue))
...
{
	public:	
				NURBSCurveValueClass(MCHAR* name) : ValueMetaClass (name) { }
...
public:
	...
	void		collect() { delete this; }
	ScripterExport void		sprin1(CharStream* s);
#	define		is_nurbstexturesurface(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(NURBSTextureSurfaceValue))
...
{
	public:	
				NURBSSurfaceValueClass(MCHAR* name) : ValueMetaClass (name) { }
	Value*		classOf_vf(Value** arg_list, int count) { return class_tag(NURBSObjectValue); }
	Value*		superClassOf_vf(Value** arg_list, int count) { return NURBSObjectValue_class.classOf_vf(NULL, 0);}
	void		collect() { delete this; }
...
public:
	...
	void		collect() { delete this; }
	ScripterExport void		sprin1(CharStream* s);
#	define		is_nurbsdisplay(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(NURBSDisplayValue))
...
public:
	...
	void		collect() { delete this; }
	ScripterExport void		sprin1(CharStream* s);
#	define		is_NURBSSurfaceApproximation(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(NURBSSurfaceApproximationValue))
...
#include "..\macros\define_implementations.h"
	...
	def_property ( merge );
	def_property ( subdivStyle );
	def_property ( minLevels );
	def_property ( maxLevels );
	def_property ( maxTris );
...
public:
	...
	void		collect() { delete this; }
	ScripterExport void		sprin1(CharStream* s);
#	define		is_nurbsset(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(NURBSSetValue))
