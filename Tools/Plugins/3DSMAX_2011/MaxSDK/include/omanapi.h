...
//! \deprecated Class IOsnapManager8 has been deprecated, thus its interface id is also deprecated
#define IID_IOsnapManager8 Interface_ID(0x77fa49fa, 0x30c73aa7)
#pragma deprecated("IID_IOsnapManager8")

 //! \deprecated Class IOSnapManager8 has been deprecated since its methods were all deprecated
MAX_DEPRECATED class IOsnapManager8 :  public IOsnapManager7
{
public:
	//@{
	/** 
	 * \deprecated Getting and setting whether the axis center is used as the start snap point is not supported as of 3ds Max 2011
	 * The snap system automatically detects when the axis center is to be used as the start snap point based on 
	 * the interactive user input via the viewports.
	*/
	MAX_DEPRECATED virtual bool IsTMAxisCenterTheDefaultStartSnapPoint() const = 0; 
	MAX_DEPRECATED virtual void SetTMAxisCenterAsDefaultStartSnapPoint(bool bEnable) = 0; 
	//@}
	MAX_DEPRECATED CoreExport static IOsnapManager8* GetInstance();
};
