...
/** \deprecated If set, the origin of the selection's transform matrix is used as start snap point, in case no other snap point has been recorded. 
 * This flag is not used by the snap system as of 3ds Max 2011. The origin of the selection's transform matrix is used as start snap point based
 * on whether the user clicks on the transform axis' center or one of its handles.
*/
#define SNAP_USE_XFORM_AXIS				(1<<15)	
#pragma deprecated("SNAP_USE_XFORM_AXIS")

/** \brief If set, the the origin of the selection's transform matrix is used as start snap point.
