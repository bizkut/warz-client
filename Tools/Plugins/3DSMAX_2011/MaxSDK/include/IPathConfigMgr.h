...
class NameEnumCallback; // deprecated
namespace MaxSDK
{
...
public:
	...
	*/
	...
	/*! \brief Deprecated: Use int IPathConfigMgr::GetAssetDirCount(MaxSDK::AssetManagement::AssetType assetType) const =0;

	Returns the number of map / xref directories in path.

	\param[in] xref specifies which type of count looking for ( xref = true / map = false)
	\return number of dirs in path
	*/
	MAX_DEPRECATED int GetMapDirCount(bool xref) const;	
	...
	*/
	...
	/*! \brief Deprecated: Use const MCHAR* IPathConfigMgr::GetAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType) const =0;

	Returns the ith bitmap / xref directory in path.
	...
	*/
	MAX_DEPRECATED const TCHAR *GetMapDir(int i, bool xref) const;
	...
	*/
	...
	/*! \brief Deprecated: use bool IPathConfigMgr::AddAssetDir(const MCHAR *dir, MaxSDK::AssetManagement::AssetType assetType)=0;

	Adds a map / xref path to the list.
	...
	\return true if added successfully
	*/
	MAX_DEPRECATED bool AddMapDir(const TCHAR *dir, bool xref);
	...
	*/
	...
	/*! \brief Deprecated: use bool IPathConfigMgr::AddAssetDir(const MCHAR *dir, MaxSDK::AssetManagement::AssetType assetType, int update) = 0;
	...
	otherwise to the map directories
	*/
	MAX_DEPRECATED bool AddMapDir(const TCHAR *dir, int update, bool xref);
	...
	*/
	...
	/*! \brief Deprecated: use bool IPathConfigMgr::DeleteAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType, int update) = 0;
	...
	\return true if Deletes successfully
	*/
	MAX_DEPRECATED bool DeleteMapDir(int i, int update, bool xref);

	/*! \brief Deprecated: use IFileResolutionManager::GetFullFilePath(const TCHAR* filePath,MaxSDK::AssetManagement::AssetType assetType)

	Resolves a given path using a predefined path search order.
	...
	*/
	MAX_DEPRECATED MSTR GetFullFilePath(const MCHAR* filePath, bool xref) const;
	...
	*/
	...
	/*! \brief Deprecated: use bool IPathConfigMgr::AddSessionAssetDir(const MCHAR *dir, MaxSDK::AssetManagement::AssetType assetType, int update)=0;
	...
	\return true if added successfully
	*/
	MAX_DEPRECATED bool AddSessionMapDir(const TCHAR *dir, int update, bool xref);
	...
	*/
	...
	/*! \brief Returns the number of bitmap or xref session directories in path.

	\param[in] xref specifies which type of count looking for ( xref = true / map = false)
	\return number of session dirs in path
	*/
	MAX_DEPRECATED int GetSessionMapDirCount(bool xref) const;
	...
	*/
	...
	/*! \brief Deprecated: use const MCHAR* IPathConfigMgr::GetSessionAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType) const =0;

	Returns the ith bitmap / xref session directory in path.
	...
	*/
	MAX_DEPRECATED const TCHAR *GetSessionMapDir(int i, bool xref) const;
	...
	*/
	...
	/*! \brief Deprecated. Use bool IPathConfigMgr::DeleteSessionAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType, int update)=0;
	...
	\return true if Deletes successfully
	*/
	MAX_DEPRECATED bool DeleteSessionMapDir(int i, int update, bool xref);
	...
	*/
	...
	/*! \brief Deprecated: use int IPathConfigMgr::GetCurAssetDirCount(MaxSDK::AssetManagement::AssetType assetType) const =0;
	...
	\return the combined total of session and permanent paths
	*/
	MAX_DEPRECATED int GetCurMapDirCount(bool xref) const;
	...
	*/
	...
	/*! \brief Deprecated: use const MCHAR* IPathConfigMgr::GetCurAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType) const =0;
	...
	\return the ith directory
	*/
	MAX_DEPRECATED const TCHAR *GetCurMapDir(int i, bool xref) const;
	...
	*/
	...
	/*! \brief Deprecated. Use void IPathConfigMgr::UpdateAssetSection(MaxSDK::AssetManagement::AssetType assetType) = 0;

	Forces the application to update itself with the currently
	set path.
	\param[in] xref Update maps or xrefs?
	*/
	MAX_DEPRECATED void UpdateMapSection(bool xref);
	...
	*/
	...
	/*! \brief Deprecated: use IFileResolutionManager::GetFullFilePath(MaxSDK::Util::Path& filePath, MaxSDK::AssetManagement::AssetType assetType)

	Resolves a given path using a predefined path search order for external files.
	...
	\remarks This method will return a UNC path name if the option controlling this is turned on. 
	*/
	MAX_DEPRECATED bool SearchForExternalFiles(MaxSDK::Util::Path& aPathToFind) const;

	/*! \brief Deprecated: use IFileResolutionManager::GetFullFilePath(MaxSDK::Util::Path& filePath, MaxSDK::AssetManagement::AssetType assetType)
	use kXRefAsset for the parameter "assetType"

	Resolves a given path using a predefined path search order for xref-ed files.
	...
	\remarks This method will return a UNC path name if the option controlling this is turned on. 
	*/
	MAX_DEPRECATED bool SearchForXRefs(MaxSDK::Util::Path& aPathToFind) const;
	...
	*/
	...
	/*! \brief Deprecated: use IPathConfigMgr::RecordInputAsset(const MaxSDK::Util::Path &originalPath,
	MaxSDK::AssetManagement::AssetType assetType,
	AssetEnumCallback &nameEnum, 
	DWORD flags) const = 0;
	*/ 
	MAX_DEPRECATED void RecordInputAsset(
	...
	*/  
	...
	//! deprecated see : virtual void RecordOutputAsset(const MaxSDK::AssetManagement::AssetUser &originalAsset,
	MAX_DEPRECATED void RecordOutputAsset(
		const MaxSDK::Util::Path &originalPath,
		AssetEnumCallback &nameEnum, 
		DWORD flags) const;
