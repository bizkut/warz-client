...
public:
	...
	/*! \brief Resolves the physical location of an asset file
		...
		\return a TSTR with an absolute path or the where the asset is located if successful; otherwise
		returns an empty TSTR if it fails to find the asset file
	*/
	virtual TSTR GetFullFilePath(const TCHAR* filePath,MaxSDK::AssetManagement::AssetType assetType, bool byPassCache = false) = 0;
	...
	*/
	...
	/*! \brief Resolves the physical location of an asset file
		...
		returns an empty TSTR if it fails to find the asset file
	*/
	virtual TSTR GetFullFilePath(const MaxSDK::AssetManagement::AssetUser& assetUser, bool byPassCache = false) = 0;
	...
	/*! \brief Resolves the physical location of an asset file
		...
		returns an empty TSTR if it fails to find the asset file
	*/
	virtual TSTR GetFullFilePath(const MaxSDK::AssetManagement::AssetId& assetId, bool byPassCache = false) = 0;
	...
	*/
	virtual MaxSDK::AssetManagement::AssetType GetAssetType(const TSTR& assetType)const = 0;
	...
	*/
	virtual const TCHAR* GetAssetTypeString(MaxSDK::AssetManagement::AssetType assetType)const = 0;
