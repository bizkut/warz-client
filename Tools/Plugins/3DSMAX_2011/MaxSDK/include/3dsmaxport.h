...
/*! \deprecated This method has been deprecated. If a deprecation warning points to this 
method, it means that your window procedure is incorrectly declared to return INT_PTR as 
opposed to LRESULT. Changing your window procedure to return LRESULT will eliminate the 
deprecation warning by allowing the compiler to resolve the call to DLSetWindowLongPtr to 
the correct version.
*/
MAX_DEPRECATED inline WNDPROC DLSetWindowLongPtr(HWND hWnd, DL_NOTQUITE_WNDPROC wndProc)
{
   // This is such a common mistake (returning int/INT_PTR instead of LRESULT) that
   // we convert it as a convenience.  If we don't do this conversion then the compiler
   // would call DLSetWindowLongPtr() with GWLP_USERDATA, which is definitely not what
   // you'd expect.  (In general you should use DLSetWIndowProc)
   return DLSetWindowProc(hWnd, reinterpret_cast<WNDPROC>(wndProc));
}
#endif
