...
namespace
{
	struct Adjacency
	{
		r3dgfxVector(unsigned int) triangle_counts;
		r3dgfxVector(unsigned int) offsets;
		r3dgfxVector(unsigned int) data;
		...
		{
            const unsigned int* beg = data.empty() ? 0 : &data[0];
			return std::make_pair(beg + offsets[vertex], beg + offsets[vertex] + triangle_counts[vertex]);
		}
	};
	...
	template <typename T> void buildAdjacency(Adjacency& adjacency, const T* indices, size_t index_count, size_t vertex_count)
	{
		...
		r3dgfxVector(unsigned int) filled_triangle_counts(vertex_count, 0);
	...
	unsigned int getNextVertexDeadEnd(r3dgfxVector(unsigned int)& dead_end, unsigned int& input_cursor, const r3dgfxVector(unsigned int)& live_triangles)
	{
	...
	unsigned int getNextVertexNeighbour(const r3dgfxVector(unsigned int)& next_candidates, const r3dgfxVector(unsigned int)& live_triangles, const r3dgfxVector(unsigned int)& cache_time_stamps, unsigned int time_stamp, unsigned int cache_size)
	{
	...
	template <typename T> void tipsify(T* destination, const T* indices, size_t index_count, size_t vertex_count, unsigned int cache_size, r3dgfxVector(unsigned int)* clusters)
	{
		...
		r3dgfxVector(unsigned int) live_triangles = adjacency.triangle_counts;

		// cache time stamps
		r3dgfxVector(unsigned int) cache_time_stamps(vertex_count, 0);

		// dead-end stack
		r3dgfxVector(unsigned int) dead_end;
		dead_end.reserve(index_count);

		// emitted flags
		r3dgfxVector(bool) emitted_flags(index_count / 3, false);
		...
		r3dgfxVector(unsigned int) next_candidates;
...
void optimizePostTLTipsify(unsigned short* destination, const unsigned short* indices, size_t index_count, size_t vertex_count, unsigned int cache_size, r3dgfxVector(unsigned int)* clusters)
{
	tipsify(destination, indices, index_count, vertex_count, cache_size, clusters);
}

void optimizePostTLTipsify(unsigned int* destination, const unsigned int* indices, size_t index_count, size_t vertex_count, unsigned int cache_size, r3dgfxVector(unsigned int)* clusters)
{
	tipsify(destination, indices, index_count, vertex_count, cache_size, clusters);
}

void BuildPreTLOptimizeMap( r3dgfxVector(int)* oMap, int* idest, const int* indices, int NumVertices, int NumIndices )
{
	oMap->resize( NumVertices );

	static r3dgfxVector(int) old2new;
	old2new.clear();
	old2new.resize( NumVertices, -1 );
	...
	for ( int id=0; id<NumIndices; id++ )
	{
		...
		if ( old2new[index]==-1 )
		{
			...
			idest[id] = old2new[index];
	}

	r3d_assert( count==oMap->size() && "data in must be same as data out" );
}
