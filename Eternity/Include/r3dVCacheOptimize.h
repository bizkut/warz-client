...
void optimizePostTLTipsify(unsigned short* destination, const unsigned short* indices, size_t index_count, size_t vertex_count, unsigned int cache_size = 16, r3dgfxVector(unsigned int)* clusters = 0);
void optimizePostTLTipsify(unsigned int* destination, const unsigned int* indices, size_t index_count, size_t vertex_count, unsigned int cache_size = 16, r3dgfxVector(unsigned int)* clusters = 0);
...
public:
	...
	r3dgfxVector(int) tri_indices; // indices to the indices that use this vertex
...
protected:
	r3dgfxVector(VertexCacheData) verts;
	r3dgfxVector(TriangleCacheData) tris;
	r3dgfxVector(int) inds;
	int best_tri; // the next triangle to add to the render list
	VertexCache vertex_cache;
	r3dgfxVector(int) draw_list;
...
template < typename T >
void PreTLOptimize( T* vdest, int* idest, const T* vertices, const int* indices, int NumVertices, int NumIndices )
{
	static r3dgfxVector(int) old2new;
	static r3dgfxVector(T) tempVerts ;
	
	old2new.clear();
	old2new.resize( NumVertices, -1 );

	tempVerts.resize( NumVertices ) ;

	// is a flush command in this case...
	if( !vdest )
	{
		r3dgfxVector(T)().swap( tempVerts ) ;
		r3dgfxVector(int)().swap( old2new ) ;
	...
	for ( int id=0; id<NumIndices; id++ )
	{
		...
		if ( old2new[index]==-1 )
		{
			tempVerts[count] = vertices[index];
			idest[id] = count;
			old2new[index] = count;
			count ++;
		}
	...
	memcpy( vdest, &tempVerts[0], sizeof(T)*NumVertices ) ;

	r3d_assert( count==tempVerts.size() && "data in must be same as data out" );
}
